...
1.  \${styledParsedYear}, \${styledParsedDoty}
2.  \${styledDecoYear}+\${styledDecoDoty}
3.  \${styledIsocYear}-\${styledIsocMd}T\${styledIsocTime}

```{ojs}
viewof inputDeco = Inputs.text({label: "Deco", placeholder: "year+day.ddd", value: "1969+306.0", maxlength: 17})
```

The `deco` parsing code in @exm-deco defines the rules for storing dates as `Decalendar ordinals` (`decos`). Notably, a `deco` consisting of a single number without a plus (+) or minus sign (-) is interpreted as a `doty` and will be appended to the current year. `Decos` support negative years and `doties`. In the example above, the range of accepted `doties` is $[-9999999999999999, 9999999999999999)$. The largest 16-digit number is not included in the acceptable range because it is automatically rounded up to 10000000000000000. `Doties` are zero padded, but years are not. There is a bug with `Year 0` and negative `doties`.
...
```{ojs}
styledParsedDoty = setStyle(ydParsed[1], d3.schemePaired[1])
styledParsedDoty
```
...
```{ojs}
styledDecoDoty = setStyle(outputDeco.split("+")[1], d3.schemePaired[1])
styledDecoDoty
```
...
```{ojs}
styledIsocMd = setStyle(outputIsoc.slice(outputIsoc.split("-")[0].length+1, outputIsoc.split("-")[0].length+6), d3.schemePaired[4])
styledIsocMd
```

```{ojs}
styledIsocTime = setStyle(outputIsoc.slice(outputIsoc.split("-")[0].length+7), d3.schemePaired[5])
styledIsocTime
```
:::
