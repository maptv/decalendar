...
`Decalendar` is a [decimal](https://en.wikipedia.org/wiki/Decimal) calendar🗓️system and `Declock` is a decimal timekeeping⏳system. Instead of months, weeks, hours, minutes, and seconds, `Decalendar` and `Declock` use a single decimal number called the day-of-the-year (`doty`).

A `doty` can represent a specific instant in a year, such as the current `doty` timestamp (`year+day.clock`): \${styledDotyDate}.\${styledDotyTime}, or a duration, such as the time since this page was loaded: \${styledDotyDiffDate}\${styledDotyDiffTime}.

`Doty` timestamps and durations are more **precise** and more **concise** than and durations that include hours, minutes, and seconds. Compare the `doty` timestamp above with the current timestamp in the `mm-ddThh:mm:ss` format: \${styledIsoDate}T\${styledIsoTime}. **The `doty` is like 5 numbers in 1!** *And that's not all!*

The [integer part](https://en.wikipedia.org/wiki/Decimal#:~:text=The%20integer%20part%20or%20integral%20part%20of%20a%20decimal%20numeral%20is%20the%20integer%20written%20to%20the%20left%20of%20the%20decimal%20separator) of the `doty` is the current `Decalendar` date: \${setStyle(dotyDate, d3.schemePaired\[1\])}. The [fractional part](https://en.wikipedia.org/wiki/Decimal#:~:text=The%20part%20from%20the%20decimal%20separator%20to%20the%20right%20is%20the%20fractional%20part%2C%20which%20equals%20the%20difference%20between%20the%20numeral%20and%20its%20integer%20part.) of the `doty` is the current `Declock` time: \${setStyle(dotyTime, d3.schemePaired\[2\])}.

The `doty` date can tell us the current season. It is now \${season\[0\]} in the Northern Hemisphere and \${season\[1\]} in the Southern Hemisphere. The \${ordinal} third of this season is associated with the \${zodiac} [Zodiac](https://en.wikipedia.org/wiki/Zodiac) sign. The `doty` [intervals](https://en.wikipedia.org/wiki/Interval_(mathematics)#Notations_for_intervals) of the seasons are $\left[20, 110\right)$🌼🍁, $\left[110, 205\right)$☀️❄️, $\left[205, 295\right)$🍁🌼, and $\left[0, 20\right)\cup\left[295, 365\right]$❄️☀️. @sec-subyear discusses seasons and other parts of the year.

The first two [digits](https://en.wikipedia.org/wiki/Numerical_digit#Computation_of_place_values) of the current `doty` date are the current `dek` number: \${styledDek}, while the last digit is the current day-of-the-`dek` (`dotd`): \${styledDotd}. `Deks` are groups of 10 days that fulfill the role of both months and weeks in `Decalendar`. The `dotd` serves as both the day-of-the-month (`dotm`) and the day-of-the-week (`dotw`) in `Decalendar`. @sec-dek explains how `deks` can replace months and weeks.

The digits of the current `doty` time provide the current `dime`: \${styledDime}, `cent`: \${styledCent}, `mil`: \${styledMil}, `phrase`: \${styledPhrase}, and `beat`: \${styledBeat}. `Dimes` are used for `Declock` time zone offsets, `cents` are 1 percent of the day, `mils` provide the right level of precision for displaying time on clocks🕰️and⌚️watches, `phrases` are like a musical🎶[phrase](https://en.wikipedia.org/wiki/Phrase_(music)), and `beats` are like a heart❤️beat. @sec-unit provides more information on the various `Decalendar` and `Declock` units.

Now it's your turn! Enter a month and `dotm` below to see 1) the equivalent `doty` date, 2) season in the Northern Hemisphere, and 3) season in the Southern Hemisphere. Try special dates like birthdays🎂and🥂anniversaries!
...
1. \${styledOutputDoty}
2. \${outputDotySeason\[0\]}
3. \${outputDotySeason\[1\]}


Similarly, you can also enter a `doty` below to get 1) the corresponding month and `dotm`, 2) season in the Northern Hemisphere, and 3) season in the Southern Hemisphere. Converting back and forth between date formats is a great way to build familiarity with `doty` dates.
...
decoLoad = unix2deco(Date.now()).slice(0, 14)
doty = deco.slice(5)
dotyLoad = decoLoad.slice(5)
dotyDiffRaw = (doty - dotyLoad).toString().split(".")
dotyDiffDate = (dotyDiffRaw[0] > 0) ? dotyDiffRaw[0] + "." : ""
dotyDiffTime = dotyDiffRaw[1].slice(0, 6)
...
outputDotySeason = doty2toty(outputDoty)
...
styledDotyDiffDate = setStyle(dotyDiffDate, d3.schemePaired[1])
styledDotyDiffTime = setStyle(dotyDiffTime, d3.schemePaired[2])
styledIsoDate = setStyle(isoDate, d3.schemePaired[5])
styledIsoTime = setStyle(isoTime, d3.schemePaired[4])
styledDek = setStyle(doty.slice(0, 2), d3.schemePaired[1])
styledDotd = setStyle(doty[2], d3.schemePaired[0])
styledDime = setStyle(dotyTime[0], d3.schemePaired[9])
styledCent = setStyle(dotyTime[1], d3.schemePaired[8])
styledMil = setStyle(dotyTime[2], d3.schemeAccent[5])
styledPhrase = setStyle(dotyTime[3], d3.schemePaired[6])
styledBeat = setStyle(dotyTime[4], d3.schemePaired[11])
styledOutputDoty = setStyle(outputDoty.toString().padStart(3, "0"), d3.schemePaired[1])
styledMonthAndDay = setStyle(doty2greg(inputDoty).map(i => i.toString().padStart(2, "0")).join("-"), d3.schemePaired[5]);
```

## Year {#sec-intro-year}

Any `doty` can be combined with a year to form a `Decalendar` `ordinal` (`deco`). Enter a year, month, `dotm`, hour, and minute below to see 1) the equivalent `deco` (`year+day.ddddd`), 2) season in the Northern Hemisphere, and 3) season in the Southern Hemisphere.
...
You can also enter a `deco` to see what it looks like after it is 1) **parsed** from a string of characters into a year and `doty`, 2) **standardized** to ensure years are integers and `doties` are positive numbers below 366 $\left[0, 366\right)$, and **converted** into 3) the `year-mm-ddThh:mm:ss` format, 4) Northern Hemisphere season, and 5) Southern Hemisphere season.

```{ojs}
//| echo: false
viewof inputDeco = Inputs.text({label: "Deco", placeholder: "year+day.ddd", value: "1969+306.0", maxlength: 16})
```

1. \${styledParsedYear}, \${styledParsedDotyDate}\${parsedDotyTimeDelimiter}\${styledParsedDotyTime}
2. \${styledDecoYear}+\${styledDecoDotyDate}\${decoDotyTimeDelimiter}\${styledDecoDotyTime}
3. \${styledIsocYear}-\${styledIsocMd}T\${styledIsocTime}
4. \${decoDotyToty\[0\]}
5. \${decoDotyToty\[1\]}
...
Both the year and the `doty` in a `deco` can be fractional. Just like [fractional days](https://en.wikipedia.org/wiki/Decimal_time#Fractional_days) can take the place of a month, `dotm`, hour, minutes, and second, fractional years can replace year, month, and `dotm` dates. In other words, fractional years can be a convenient way to express a date in a single number. The current `Decalendar` year date is \${styledYearDate}.
...
outputDatetime = unix2deco(inputDatetime - tzOffsetInMs)
dtDotyDateRaw = outputDatetime.slice(5, 8)
dtDotyTimeRaw = outputDatetime.slice(9, 14)
dtDotyTimeDelimiter = dtDotyTime ? "." : ""
ydParsed = deco2doty(inputDeco)
parsedDotyTime = ydParsed[1].toString().split(".")[1]
parsedDotyTimeDelimiter = parsedDotyTime ? ", " : ""
outputDeco = doty2deco(...ydParsed)
decoDoty = outputDeco.split("+")[1]
decoDotyDate = decoDoty.split(".")[0]
decoDotyTime = decoDoty.split(".")[1]
decoDotyTimeDelimiter = decoDotyTime ? "." : ""
outputIsoc = deco2isoc(outputDeco)
yearDate = Math.ceil(unix2year(unix) * 1e3) / 1e3
dtDotyToty = doty2toty(dtDotyDateRaw)
decoDotyToty = doty2toty(decoDotyDate)
```

```{ojs}
decoYear = setStyle(outputDeco[0], d3.schemePaired[10])
dtYear = setStyle(outputDatetime.slice(0, 4), d3.schemePaired[10])
dtDotyDate = setStyle(dtDotyDateRaw, d3.schemePaired[1])
dtDotyTime = dtDotyTimeRaw ? setStyle(dtDotyTimeRaw, d3.schemePaired[2]) : ""
styledParsedYear = setStyle(ydParsed[0], d3.schemePaired[10])
styledParsedDotyDate = setStyle(ydParsed[1].toString().split(".")[0], d3.schemePaired[1])
styledParsedDotyTime = parsedDotyTime ? setStyle(parsedDotyTime, d3.schemePaired[2]) : ""
styledDecoYear = setStyle(outputDeco.split("+")[0], d3.schemePaired[10])
styledDecoDotyDate = setStyle(decoDoty.split(".")[0], d3.schemePaired[1])
styledDecoDotyTime = decoDotyTime ? setStyle(decoDotyTime, d3.schemePaired[2]) : ""
...
styledYearDate = setStyle(yearDate, d3.schemePaired[10])
```

## Time Zone {#sec-intro-zone}

## Dek {#sec-intro-dek}

As mentioned in @sec-intro-doty, `deks` function as both months and weeks in `Decalendar`. A `dek` consists of 2 groups of 5 days called `pents.` Each `pent` can follow a sequence of workdays and restdays called a `pently` schedule. The code in @exm-glance generates @fig-schedules which compares the typical weekly schedule and the `Schedule` `3` `pently` schedule. Like other `pently` schedules, `Schedule` `3` is named after the number of workdays it contains. The 3 workdays in `Schedule` `3` are followed by a 2-day `pentend`, the `Decalender` equivalent of a weekend.
