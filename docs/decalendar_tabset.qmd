...
`Doty` numbers can be combined with years to form `Decalendar` ordinal (`deco`) dates (`year+day`) and timestamps (`year+day.ddddd`).
Enter a timestamp in `year-mm-dd hh:mm` format below to see the equivalent `deco` timestamp: \${dtYear}+\${dtDotyDate}.\${dtDotyTime}.
...
```{ojs}
dtDotyTime = setStyle(outputDatetime.slice(9), d3.schemePaired[2])
dtDotyTime
```
