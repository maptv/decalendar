...
The [integer part](https://en.wikipedia.org/wiki/Decimal#:~:text=The%20integer%20part%20or%20integral%20part%20of%20a%20decimal%20numeral%20is%20the%20integer%20written%20to%20the%20left%20of%20the%20decimal%20separator) of the `doty` is the current `Decalendar` date:
\${setStyle(dotyDate, d3.schemePaired[0])}.
The [fractional part](https://en.wikipedia.org/wiki/Decimal#:~:text=The%20part%20from%20the%20decimal%20separator%20to%20the%20right%20is%20the%20fractional%20part%2C%20which%20equals%20the%20difference%20between%20the%20numeral%20and%20its%20integer%20part.) of the `doty` is the current `Declock` time:
.\${setStyle(dotyTime, d3.schemePaired[3])}.
...
The digits of the current `doty` time provide the current `dime`: ${setStyle(dime, d3.schemePaired[2])}, `cent`: ${setStyle(cent, d3.schemePaired[3])}, `mil`: ${setStyle(mil, d3.schemeAccent[5])}, `phrase`: ${setStyle(phrase, d3.schemeAccent[8])}, and `beat`: ${setStyle(beat, d3.schemeAccent[9])}. `Dimes` are used for `Declock` time zone offsets, `cents` are 1 percent of the day, `mils` provide the right level of precision for displaying time on clocks🕰️and⌚️watches, `phrases` are like a musical🎶[phrase](https://en.wikipedia.org/wiki/Phrase_(music)), and `beats` are like a heart❤️beat.
...
Now it's your turn! Enter a month and `dotm` below to see the equivalent `doty` date: ${setStyle(outputDoty, d3.schemePaired[0])}. Try special dates like birthdays🎂and🥂anniversaries!
...
You can also enter a `doty` below to get the current month and `dotm`: ${setStyle(monthAndDay, d3.schemePaired[4])}. Converting back and forth between date formats is a great way to build familiarity with `doty` dates.
...
```{ojs}
monthAndDay = doty2greg(inputDoty).map(i => i.toString().padStart(2, "0")).join("-");
monthAndDay
```
:::
...
`Doty` numbers can can be combined with years to form `Decalendar` ordinal (`deco`) dates (`year+day`) and timestamps (`year+day.ddddd`). The current `deco` timestamp is
${setStyle(decoYear, d3.schemePaired[7])}
${setStyle(dotyDate, d3.schemePaired[0])}
${setStyle(dotyTime, d3.schemePaired[3])}. Enter a timestamp in `year-mm-dd hh:mm` format below to see the equivalent `deco` timestamp: 
${setStyle(dtYear, d3.schemePaired[7])}
${setStyle(dtDotyDate, d3.schemePaired[0])}
${setStyle(dtDotyTime, d3.schemePaired[3])}.
You can also enter a `year-mm-dd` date and `hh:mm` time to see the synonymous `deco` date and time (`year+day.ddddd`): ${outputDatetime}.
...
yearFrac = Math.ceil(unix2year(unix) * 1e3) / 1e3
```
:::
