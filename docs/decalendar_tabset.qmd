...
`Doty` numbers are slightly more **precise** and much more **concise** than dates and times in the form of a month, day, hour, minute, and second. Compare the current `doty` to the current date and time in `mm-ddThh:mm:ss` format: ${isoDate}${isoTime}. **The `doty` is like 5 numbers in 1!** *And that's not all!*
...
Now it's your turn! Enter a month and `dotm` below to see the equivalent `doty` date: ${outputDoty.toString().padStart(3, "0")}. Try special dates like birthdays🎂and🥂anniversaries!
...
You can also enter a `doty` below to get the current month and `dotm`: ${monthAndDay.map(i => i.toString().padStart(2, "0")).join("-")}. Converting back and forth between date formats is a great way to build familiarity with `doty` dates.
...
`Doty` numbers can can be combined with years to form `Decalendar` ordinal (`deco`) dates (`year+day`) and timestamps (`year+day.ddddd`). The current `deco` timestamp is ${year}+${dotyDate}.${dotyTime}. Enter a timestamp in `year-mm-dd hh:mm` format below to see the equivalent `deco` timestamp: ${dtYear.toString().padStart(4, "0")}+${dtDotyDate.toString().padStart(3, "0")}.${dtDotyTime.toString().padStart(5, "0")}. You can also enter a `year-mm-dd` date and `hh:mm` time to see the synonymous `deco` date and time (`year+day.ddddd`): ${dtYear}.
