...
Any `doty` can be combined with a year to form a `Decalendar ordinal` (`deco`). Enter a year, month, `dotm`, hour, and minute below to see the equivalent `deco` (`year+day.ddd`): \${dtYear}+\${dtDotyDate}.\${dtDotyTime}.
...
The `deco` parsing code in @exm-deco defines the rules for storing dates as `Decalendar ordinals` (`decos`). Notably, a `deco` consisting of a single number without a plus (+) or minus sign (-) is interpreted as a `doty` and will be appended to the current year. `Decos` can be entered with negative years and `doties`, but are always displayed with positive `doties`. `Doties` are zero padded to 3 digits, but years are not. If the input year is not an integer gg
