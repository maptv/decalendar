...
\${setStyle(dotyDate, d3.schemePaired[1])}.
...
.${setStyle(dotyTime, d3.schemePaired[2])}.

`Doty` numbers are slightly more **precise** and much more **concise** than dates and times in the form of a month, day, hour, minute, and second. Compare the current `doty` to the current date and time in `mm-ddThh:mm:ss` format:
\${styledIsoDate}T${styledIsoTime}. **The `doty` is like 5 numbers in 1!** *And that's not all!*

The first two [digits](https://en.wikipedia.org/wiki/Numerical_digit#Computation_of_place_values) of the current `doty` date are the current `dek` number:
${styledDek}, while the last digit is the current day-of-the-`dek` (`dotd`):
${styledDotd}. `Deks` are groups of 10 days that fulfill the role of both months and weeks in `Decalendar`. The `dotd` serves as both the day-of-the-month (`dotm`) and the day-of-the-week (`dotw`) in `Decalendar`.

The digits of the current `doty` time provide the current `dime`:
${styledDime}, `cent`:
${styledCent}, `mil`:
${styledMil}, `phrase`:
${styledPhrase}, and `beat`:
${styledBeat}.
`Dimes` are used for `Declock` time zone offsets, `cents` are 1 percent of the day, `mils` provide the right level of precision for displaying time on clocks🕰️and⌚️watches, `phrases` are like a musical🎶[phrase](https://en.wikipedia.org/wiki/Phrase_(music)), and `beats` are like a heart❤️beat.
...
function dote2doty(days = 719468) {
  ...
  return [
      yotc + era * 400,
      Math.floor(dotc) + Math.floor(yotc / 100) - yotc * 365 - Math.floor(yotc / 4)
      + (days.toString().includes(".") && Number(days.toString().split(/(?=\.)/)[1]))
  ];
}
...
```{ojs}
styledIsoDate = setStyle(isoDate, d3.schemePaired[4])
styledIsoDate
```
...
```{ojs}
styledIsoTime = setStyle(isoTime, d3.schemePaired[5])
styledIsoTime
```
...
```{ojs}
styledMil = setStyle(dotyTime[2], d3.schemePaired[7])
styledMil
```
...
```{ojs}
monthAndDay = setStyle(doty2greg(inputDoty).map(i => i.toString().padStart(2, "0")).join("-"), d3.schemePaired[4]);
monthAndDay
```
:::
...
`Doty` numbers can be combined with years to form `Decalendar` ordinal (`deco`) dates (`year+day`).
Enter a year, month, `dotm`, hour, and minute below to see the equivalent `deco` timestamp (`year+day.ddd`): \${dtYear}+\${dtDotyDate}.\${dtDotyTime}.

```{ojs}
viewof inputDatetime = Inputs.datetime({label: "Date", value: "1970-01-01T00:00"})
```

You can also enter a `deco` timestamp to see what it looks like

- parsed: [\${ydParsed[0]}, \${ydParsed[1]}],
- standardized: [\${ydStandardized[0]}, \${ydStandardized[1]}], and
- converted: ${outputIsoc}.

```{ojs}
viewof inputDeco = Inputs.text({label: "Deco", placeholder: "year+day.ddd", value: "1969+306.0", maxlength: 14})
```
...
function doty2isoc(year = 1969, doty = 306) {
    return new Date(doty2unix(year, doty)).toISOString().slice(0, 19)
}
```

```{ojs}
decoYear = setStyle(deco.slice(0, 4), d3.schemePaired[10])
decoYear
```
...
```{ojs}
dtDotyTime = setStyle(outputDatetime.slice(9, 12), d3.schemePaired[2])
dtDotyTime
```
...
```{ojs}
ydStandardized = doty2doty(...ydParsed)
ydStandardized
```

```{ojs}
outputIsoc = doty2isoc(...ydStandardized)
outputIsoc
```
