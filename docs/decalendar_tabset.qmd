...
`Decalendar` is a [decimal](https://en.wikipedia.org/wiki/Decimal) calendar🗓️system and `Declock` is a decimal timekeeping⏳system. Instead of months, weeks, hours, minutes, and seconds, `Decalendar` and `Declock` use a single decimal number called the day-of-the-year (`doty`). The current `doty` date and time: \${styledDotyDate}.\${styledDotyTime}.

Days-of-the-year (`doties`) are slightly more **precise** and much more **concise** than dates and times in the form of a month, day, hour, minute, and second. Compare the current `doty` to the current date and time in `mm-ddThh:mm:ss` format: \${styledIsoDate}T\${styledIsoTime}. **The `doty` is like 5 numbers in 1!** *And that's not all!*
...
While they are not very useful in `decos`, 
Multiples of a fifth (⅕, .2) of a year are of interest because they result in `doties` that are multiples of 73.
...
outputDeco = doty2doty(...ydParsed)
decoDoty = outputDeco[1]
decoDotyTime = decoDoty.toString().split(".")[1]
...
styledDecoYear = setStyle(outputDeco[0], d3.schemePaired[10])
styledDecoDotyDate = setStyle(decoDoty.toString().split(".")[0], d3.schemePaired[1])
