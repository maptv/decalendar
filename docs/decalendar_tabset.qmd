...
`Decalendar` is a [decimal](https://en.wikipedia.org/wiki/Decimal) calendar🗓️system and `Declock` is a decimal timekeeping⏳system. Instead of months, weeks, hours, minutes, and seconds, `Decalendar` and `Declock` use a single decimal number called the day-of-the-year (`doty`). @exm-doty shows code that can be used to obtain the current `doty` date and time: ${setStyle(dotyDate, d3.schemePaired[0])}.${setStyle(dotyTime, d3.schemePaired[3])}.

The [integer part](https://en.wikipedia.org/wiki/Decimal#:~:text=The%20integer%20part%20or%20integral%20part%20of%20a%20decimal%20numeral%20is%20the%20integer%20written%20to%20the%20left%20of%20the%20decimal%20separator) of the `doty` is the current `Decalendar` date: ${setStyle(dotyDate, d3.schemePaired[0])}. The [fractional part](https://en.wikipedia.org/wiki/Decimal#:~:text=The%20part%20from%20the%20decimal%20separator%20to%20the%20right%20is%20the%20fractional%20part%2C%20which%20equals%20the%20difference%20between%20the%20numeral%20and%20its%20integer%20part.) of the `doty` is the current `Declock` time: .${setStyle(dotyTime, d3.schemePaired[3])}.

`Doty` numbers are slightly more **precise** and much more **concise** than dates and times in the form of a month, day, hour, minute, and second. Compare the current `doty` to the current date and time in `mm-ddThh:mm:ss` format: ${setStyle(isoDate, d3.schemePaired[4])}${setStyle(isoTime, d3.schemePaired[5])}. **The `doty` is like 5 numbers in 1!** *And that's not all!*

The first two [digits](https://en.wikipedia.org/wiki/Numerical_digit#Computation_of_place_values) of the current `doty` date are the current `dek` number: ${setStyle(dek, d3.schemePaired[0])}, while the last digit is the current day-of-the-`dek` (`dotd`): ${setStyle(dotd, d3.schemePaired[1])}. `Deks` are groups of 10 days that fulfill the role of both months and weeks in `Decalendar`. The `dotd` serves as both the day-of-the-month (`dotm`) and the day-of-the-week (`dotw`) in `Decalendar`.

The digits of the current `doty` time provide the current `dime`: ${setStyle(dime, d3.schemePaired[2])}, `cent`: ${setStyle(cent, d3.scheme[3])}, `mil`: ${setStyle(mil, d3.schemeAccent[5])}, `phrase`: ${setStyle(phrase, d3.schemeAccent[8])}, and `beat`: ${setStyle(beat, d3.schemeAccent[9])}. `Dimes` are used for `Declock` time zone offsets, `cents` are 1 percent of the day, `mils` provide the right level of precision for displaying time on clocks🕰️and⌚️watches, `phrases` are like a musical🎶[phrase](https://en.wikipedia.org/wiki/Phrase_(music)), and `beats` are like a heart❤️beat.
...
Now it's your turn! Enter a month and `dotm` below to see the equivalent `doty` date: ${setStyle(outputDoty.toString().padStart(3, "0"), d3.schemePaired[0])}. Try special dates like birthdays🎂and🥂anniversaries!
...
You can also enter a `doty` below to get the current month and `dotm`: ${setStyle(monthAndDay.map(i => i.toString().padStart(2, "0")).join("-"), d3.schemePaired[4])}. Converting back and forth between date formats is a great way to build familiarity with `doty` dates.
...
## Year {#sec-summary-year}

`Doty` numbers can can be combined with years to form `Decalendar` ordinal (`deco`) dates (`year+day`) and timestamps (`year+day.ddddd`). The current `deco` timestamp is ${setStyle(year, d3.schemePaired[7])}+${setStyle(dotyDate, d3.schemePaired[0])}.${setStyle(dotyTime, d3.schemePaired[3])}. Enter a timestamp in `year-mm-dd hh:mm` format below to see the equivalent `deco` timestamp: ${setStyle(dtYear.toString().padStart(4, "0"), d3.schemePaired[7])}+${setStyle(dtDotyDate.toString().padStart(3, "0"), d3.schemePaired[0])}.${setStyle(dtDotyTime.toString().padStart(5, "0"), d3.schemePaired[3])}. You can also enter a `year-mm-dd` date and `hh:mm` time to see the synonymous `deco` date and time (`year+day.ddddd`): ${decoDatetime}.
...
```{ojs}
outputDeco = unix2deco(inputDate).slice(0, 8)
outputDeco
```

```{ojs}
outputDatetime = unix2deco(inputDatetime)
outputDatetime
```

```{ojs}
dtYear = outputDatetime.slice(0, 4)
dtYear
```

```{ojs}
dtDotyDate = outputDatetime.slice(5, 8)
dtDotyDate
```

```{ojs}
dtDotyTime = outputDatetime.slice(9)
dtDotyTime
```
