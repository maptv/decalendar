...
Interestingly, years from negative `deco` dates do not need any adjustment. The last day of `Year 1999` is `1999+365` or `2000-001` in `Decalendar` and February 29, 2000 in the Gregorian calendar. `Year 1999` is a leap year from the perspective of the positive `deco` date, because it's . The first day of `Year 1999` is `1999+000` or `2000-366` in `Decalendar` and March 1, 1999 in the Gregorian calendar. `Year 2000` is thus a leap year from the point-of-view  of the negative `deco` date, because pthe negative `deco` date and the while `Year 2000` is the leap year from the of the Gregorian calendar date. 
...
Like Julian dates, `dote` dates are very useful for date and time calculations. To find the number of fractional days in between two dates, we first convert both into `dote` numbers and subtract the earlier `dote` from the later `dote`. @eq-dote and @exm-jd show how a year and a `doty` can be turned into a `dote`. The conversion of a `dote` into a year and `doty` requires calculations adapted from the ["civil_from_days" algorithm](http://howardhinnant.github.io/date_algorithms.html#civil_from_days) from [@hinnant2014]. Briefly, the `dote` is used to obtain the [solar cycle](https://en.wikipedia.org/wiki/Solar_cycle_(calendar)#:~:text=400%2Dyear%20cycle%20of%20the%20Gregorian%20calendar%20with%20respect%20to%20the%20week.) in @eq-cycl, which is then plugged into @eq-dotc and @eq-yotc to calculate the `day-of-the-cycle` in (`dotc`) and the `year-of-the-cycle` (`yotc`) in @eq-yotc. @eq-year takes the `yotc` and the solar cycle and yields the year, while @eq-doty generates the `doty` from the `yotc` and the `dotc`.
