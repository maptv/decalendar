...
### ISO 8601 time zones {#sec-isoz}

ISO 8601 timestamps will often have a `Z` at the end (`year-mm-ddThh:mm:ssZ`). This `Z` is a [military time zone code](https://en.wikipedia.org/wiki/Military_time_zone#Description) that represents the [UTC+00:00](https://en.wikipedia.org/wiki/UTC+00:00) time zone, which is the basis of [Coordinated Universal Time (UTC)](https://en.wikipedia.org/wiki/Coordinated_Universal_Time). `Declock` times that are synonymous with UTC can end in `Z` (`year+day.dddddZ`) or a `+0` (`year+day.ddddd+0`). Noon UTC can be written `.5Z` or `.5+0` in `Declock` and 12:00:00Z, 120000Z, 12:00:00+00, or 120000+00 as per ISO 8601. The code in @exm-isoz converts between UTC offsets and military time zone codes. @sec-zone provides more information on `Declock` time zones.
...
### ISO 8601 timestamp parsing {#sec-isop}

To extract the components of the timestamps described above, we can use the `parse_dec` and `parse_iso` functions in @exm-isop. Parsing timestamps is the first step before any later processes such as timestamp arithmetic or conversion between timestamp formats. The `parse_date` and `parse_doty` functions use [regular expressions](https://en.wikipedia.org/wiki/Regular_expression) to handle incomplete timestamps with or without delimiters. These regular expressions are in essence succinct definitions of the ISO 8601 and `Decalendar` timestamp formats. The years in both timestamp formats must be 4 digits long (`\d{4}`).

The `parse_doty` function returns a year, a `doty`, a fraction day time, and a fractional day time zone offset, while the return value of `parse_date` function consists of a year, a month, a `dotm`, hours, minutes, and seconds. Military time zone codes are processed with the `zone2hour` function.
...
(
    f"{doty2year():>04}-{'-'.join(map(lambda i: str(i).rjust(2, '0'), doty2date()))}"
    f"T{':'.join(map(lambda i: str(round(i)).rjust(2, '0'), doty2time(0)))}{hour2zone()}"
)
```
...
function date2link(calendar = "google") {
    switch( calendar. ){
    return 'https://www.google.com/calendar/event?action=TEMPLATE'
        ...
        + `&location=${location}`
        + `&dates=${start}/${stop}`
        + `&sprop=website:` + encodeURIComponent(data.url)
        + `&sprop=name:` + encodeURIComponent(data.organizer.name);
}
function time2doty(hours = 1, minutes = 0, seconds = 0) {
    return hours / 24 + minutes / 1440 + seconds / 86400
}
...
Building on the concept of array slicing (`start:stop:step`), `Decalendar` introduces a new technique called "array spreading" (`start>span>split>space`) and even allows for nesting and mixing of `slices` and `spreads`. The various expressions that represent dates, times, timestamps, and time intervals are collectively called `Decalendar strings`.

(https://calendar.google.com/calendar/render?action=TEMPLATE&dates=20230930T131500Z/20230930T134500Z)

(https://outlook.live.com/calendar/0/action/compose?allday=false&enddt=2023-09-30T13:45:00+00:00&path=/calendar/action/compose&rru=addevent&startdt=2023-09-30T13:15:00+00:00)

(https://outlook.office.com/calendar/action/compose?allday=false&enddt=2023-09-30T13:45:00+00:00&path=/calendar/action/compose&rru=addevent&startdt=2023-09-30T13:15:00+00:00)

(https://calendar.yahoo.com/?dur=&et=20230930T134500Z&st=20230930T131500Z&v=60)
...
y, d, t = unix2doty(time())
f"{y:>04}+{d:>03}.{round(t * 1e5):>05}{hour2zone()}"
```
