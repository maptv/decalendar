...
```python
def function isoo2year(year = 1970, day = 1):
    return year - (day < (60 + is_leap(year - 1)))

def isoo2doty(year=1970, day=1):
    return (day + 305 - is_leap(year)) % 365

f"{isoo2year()}+{isoo2doty()}"

def doty2isoo(year=1970, doty=0):
    return (doty + 60 + is_leap(year + 1)) % 365
}

f"{doty2year()}+{doty2isoo()}"
```
...
paste0(isoo2year(), "+", isoo2doty())

doty2isoo <- function(year = 1970, doty = 0) {
    (doty + 60 + is_leap(year + 1)) %% 365
}

paste0(doty2year(), "+", doty2isoo())
```
...
ISO 8601 timestamps will often have a `Z` at the end (`year-mm-ddThh:mm:ssZ`). This `Z` is a [military time zone code](https://en.wikipedia.org/wiki/Military_time_zone#Description) that represents the [UTC+00:00](https://en.wikipedia.org/wiki/UTC%2B00:00) time zone, which is the basis of [Coordinated Universal Time (UTC)](https://en.wikipedia.org/wiki/Coordinated_Universal_Time). `Declock` times that are synonymous with UTC should end in `Z` (`year+day.dddddZ`). Noon UTC can be written `.5Z` in `Declock` and 12:00:00Z or 12:00:00+00:00 as per ISO 8601. The code in @exm-isoz converts between UTC offsets and military time zone codes.

`Declock` times can also end with a numeric offset (`year+day.ddddd+o`) to indicate a `Declock` time zone. `Zone 0`, which is represented by the numeric offset `+0`, is the `Declock` time zone that is closest to UTC. `Declock` time zones do not include [leap seconds](https://en.wikipedia.org/wiki/Leap_second), therefore `Zone 0` is currently 27 seconds (`.0003125`) behind UTC. Noon in `Zone 0` can be written `.5+0`, `.5003125Z`, 12:00:27Z, and 12:00:27+00:00. @sec-unix shows how `Decalendar` functions without leap seconds and @sec-zone provides more information on `Declock` time zones.
...
#### Standard Time to Decimal Time Conversion {#sec-isoc}

The formula for conversion of standard time to `Declock` time is shown in @eq-isot. The value of `x` in @eq-isot can be modified to obtain different units, which are displayed in @tbl-isot. To convert `Declock` time into standard time, we first convert into hours using @eq-isot2 and the appropriate `x` value from @tbl-isot. Then, we convert hours into minutes with @eq-isot3 and minutes into seconds with @eq-isot4. The  `time2doty` and `doty2time` functions in @exm-isot convert between standard time and `Declock` time. To create a full `Decalendar` timestamp, we can use the `date2year`, `date2doty`, `time2doty`, and `hour2zone` functions as shown in @exm-isot. Similarly, @exm-isot also shows how to create an ISO 8601 calendar date timestamp with the `doty2year`, `doty2date`, `doty2time`, and `hour2zone` functions.
