...
To extract the components of the timestamps described above, we can use the code in @exm-isop. The `parse_date` and `parse_doty` functions can handle complete or partial timestamps with or without delimiters thanks to the string matching capabilities of [regular expressions](https://en.wikipedia.org/wiki/Regular_expression). The `parse_doty` function returns a year, a `doty`, a fraction day time, and a fractional day time zone offset, while the return value of `parse_date` function consists of a year, a month, a `dotm`, hours, minutes, and seconds. Military time zone codes are processed with the `zone2hour` function.
