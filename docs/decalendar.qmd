...
$$(JD − 2440587.5) \cdot 86400$$ {#eq-unix}
...
$$yoe = \lfloor doe - doe \div 1460 + doe \div 36524 - doe \div 146096) \div 365 \rfloor$$ {#eq-yoe}

$$year = yoe + era * 400$$ {#eq-year}

$$doty = \lfloor doe - (365 \cdot yoe + yoe \div 4 - yoe \div 100) \rfloor$$ {#eq-doty}

To obtain fractional days, we have to extend this calculation with a new algorithm that sums up all of the days in previous years and subtracts this sum from `days` to obtain the current `Decalendar` timestamp as shown in @eq-time or @eq-iver. The @exm-time converts UNIX time into a `Decalendar` date or a `Decalendar` timestamp. The last line in @exm-time obtains the current `Decalendar` date by passing the current UNIX timestamp to the function. If we pass 0 to the function (`unix2doty(0)`) the result should be the UNIX Epoch: `1969+306`. To see @exm-time in action, visit this [CodePen](https://codepen.io/maptv/pen/oNJZeea) which displays the current timestamps in the `Decalendar` (`year+day.ddddd`) and ISO 8601 (`year-mm-ddThh:mm:ss`) timestamp formats.

### UNIX time conversion to Decalendar timestamp

$$time = year \cdot 365 + \sum_{n=1}^{year} \begin{cases}1&{\text{if  } y \mod 4 = 0 \land y \mod 100 \neq 0 \lor y \mod 400 = 0;}\\0&{\text{otherwise.}}\end{cases}$$ {#eq-time}

$$time = year \cdot 365 + \sum_{n=1}^{year} [y \mod 4 = 0 \land y \mod 100 \neq 0 \lor y \mod 400 = 0]$$ {#eq-iver}
