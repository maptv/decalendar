...
Every date in the Gregorian calendar has a `Decalendar` day of the year (`doty`) equivalent. The first day of the `Decalendar` year, `Day 0`, is March 1 in the Gregorian calendar. Starting the year on March 1 positions leap days, `Day 365` in `Decalendar` and February 29 in the Gregorian calendar, at the end of the year. `Decalendar` leap years therefore occur one year earlier than Gregorian calendar leap years. To check if a year is a `Decalendar` leap year, we add 1 to the year and proceed with the [`is_leap`](http://howardhinnant.github.io/date_algorithms.html#is_leap) algorithm, described by [Howard Hinnant](https://howardhinnant.github.io/) in [_chrono-Compatible Low-Level Date Algorithms_](http://howardhinnant.github.io/date_algorithms.html) [-@hinnant2014] and translated from C++ into $\LaTeX$ in @eq-gregl and into [JavaScript](https://tc39.es/ecma262), [Julia](https://julialang.org), [Python](https://www.python.org), and [R](https://www.r-project.org/) in @exm-gregl.

$$y \mod 4 = 0 \land y \mod 100 \neq 0 \lor y \mod 400 = 0$$ {#eq-gregl}
...
`Decalendar` uses `doty` numbers as dates instead of month and day-of-the-month (`dotm`) numbers, but if required, Gregorian calendar dates can provided in the `m+dd` format, where `m` is the single-digit, [hexadecimal](https://en.wikipedia.org/wiki/Hexadecimal)-encoded month and `dd` is the double-digit, [zero-based](https://en.wikipedia.org/wiki/Zero-based_numbering) `dotm`. The ordinal positions of September, October, November, and December in `Decalendar` match the [numeral prefixes](https://en.m.wikipedia.org/wiki/Numeral_prefix#Table_of_number_prefixes_in_English) in their names (Sep=7, Oct=8, Nov=9, Dec=10) and can be decremented to obtain their `m` values (Sep=6, Oct=7, Nov=8, Dec=9).

To obtain `m` values from Gregorian calendar month, we subtract 3 if the month number is greater than two and add 9 if not ($m = \begin{cases}month - 3&{\text{if } month > 2;}\\month + 9&{\text{otherwise.}}\end{cases}$). m = mp < 10 ? mp+3 : mp-9 sWe then hexadecimal-encode the result. remainder ($m = (month + 9) \mod 12$). The inverse function () @exm-gregm shows 

m = mp < 10 ? mp+3 : mp-9

In `Decalendar`, January is represented by `A` and February is represented by `B` (mnemonic: `jAn`=January, `feB`=February). During January and February, the `Decalendar` year is 1 less than the Gregorian calendar year ($dyear = gyear - [m > 9]$). @tbl-mval shows the positive and negative `m` values of each Gregorian calendar month. @sec-dotm provides more information on how positive and negative `m` values are used in the `m+dd` format.
...
function date2doty(year=1970, month=1, day=1)
    year = year - (month <= 2)
    doty = Int(round((153 * (month > 2 ? month - 3 : month + 9) + 2) / 5 + day - 1))
    "$(lpad(year,4,'0'))+$(lpad(doty,3,'0'))"
end
...
```python
def date2doty(year=1970, month=1, day=1):
    year -= month <= 2
...
```r
date2doty <- function(year = 1970, month = 1, day = 1) {
    year = year - (month <= 2)
    doty = round((153 * (ifelse(month > 2, month - 3, month + 9)) + 2) / 5 + day - 1)
    paste0(sprintf("%04d", year), "+", sprintf("%03d", doty))
}
...
### ISO 8601 dates {#sec-isod}

The `m+dd` format described in @sec-gregm is similar to the [calendar date](https://en.wikipedia.org/wiki/ISO_8601#Calendar_dates) format (year-mm-dd) of the [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601#Ordinal_dates) international standard for [dates](https://en.wikipedia.org/wiki/ISO_8601#Dates) and [times](https://en.wikipedia.org/wiki/ISO_8601#Times). `Decalendar` dates (`year±day`) are most similar to ISO 8601 [ordinal dates](https://en.wikipedia.org/wiki/ISO_8601#Ordinal_dates) (year-day). In fact, the `Decalendar` `doty` number can be obtained from the ISO 8601 ordinal day number using the calculation shown in @eq-ord and @exm-ord. This calculation essentially shifts the ordinal date number to be 60 days earlier.
