...
date: last-modified
date-format: "[Last updated on] %YYYY%+%DDD%"
filters:
  - date.lua
format:
  html:
...
In the Gregorian calendar, dates are like a set of coordinates, where the month and the day-of-the-month (`dotm`) are like longitude and latitude in the [geographic coordinate system](https://en.wikipedia.org/wiki/Geographic_coordinate_system) or x and y in the [Cartesian coordinate system](https://en.wikipedia.org/wiki/Cartesian_coordinate_system). `Decalendar` provides two coordinates in one number: the day-of-the-year (`doty`). The first two digits of the `doty` is the `dek` number. `Deks` are groups of 10 days that fulfill the role of both months and weeks in `Decalendar`. The last digit of the `doty` is the day-of-the-`dek` (`dotd`) number, which serves as both the `dotm` and the day-of-the-week (`dotw`) in `Decalendar`. @tbl-conv shows the `doty` equivalents of all Gregorian calendar dates. To find a specific date in @tbl-conv, first find the month among the columns (think of the month as an x-axis value) and then find the `dotm` among the rows (the day is like a y-axis value).
...
```julia
function date2doty(year=1969, month=1)
    year - (month < 3)
end
...
```r
date2doty <- function(year = 1970, month = 1) {
    year - (month < 3)
}

date2doty()

doty2date <- function(year = 1969, doty = 306) {
    year + (doty > 305)
}
...
`Decalendar` dates (`year+day`) are most similar to [ordinal dates](https://en.wikipedia.org/wiki/ISO_8601#Ordinal_dates) (`year-day`) specified by the [ISO 8601 international standard](https://en.wikipedia.org/wiki/ISO_8601) for [dates](https://en.wikipedia.org/wiki/ISO_8601#Dates) and [times](https://en.wikipedia.org/wiki/ISO_8601#Times). In fact, the `Decalendar` `doty` number can be obtained from the ISO 8601 ordinal day number using the calculation shown in @eq-isoo and @exm-isoo This calculation essentially shifts the ordinal date by 60 days to account for the 2-month difference between `Decalendar` and the Gregorian calendar. We use the `is_leap` function from @exm-gregl in @exm-isoo to account for the fact that the Gregorian calendar leap year is not at the end of the year.

$$
(isoordinal + 305 - is_leap(year)) \mod 365 \\
(doty + 60 + is_leap(year + 1)) \mod 365
$$ {#eq-isoo}
...
As mentioned in @sec-coor, `Decalendar` uses `doty` numbers as dates instead of month and day-of-the-month (`dotm`) numbers, but if required, Gregorian calendar dates can be provided in the `m+dd` format, where `m` is the single-digit, [hexadecimal](https://en.wikipedia.org/wiki/Hexadecimal)-encoded month and `dd` is the double-digit, [zero-based](https://en.wikipedia.org/wiki/Zero-based_numbering) `dotm`. The `m+dd` format is similar to the [ISO8601 calendar date](https://en.wikipedia.org/wiki/ISO_8601#Calendar_dates) format (year-mm-dd).

To convert a double-digit Gregorian calendar month number (`mm`) into a single-digit `Decalendar` `m` value, we subtract 3 if `mm` is greater than 2, add 9 if not ($m = \begin{cases}mm - 3&{\text{if } month > 2;}\\mm + 9&{\text{otherwise.}}\end{cases}$), and then encode into hexadecimal (Base16). To do the inverse (convert `m` to `mm`), we decode from hexadecimal, add 3 to `m` values less than 10 and subtract 9 from other `m` values ($m = \begin{cases}month + 3&{\text{if } month < 10;}\\month - 9&{\text{otherwise.}}\end{cases}$. @exm-gregm includes code for round-trip conversion of `mm` and `m` values, including hexadecimal encoding and decoding.

)= mp < 10 ? mp+3 : mp-9 sWe then hexadecimal-encode the result. remainder ($m = (month + 9) \mod 12$). The inverse function () shows 
 

m = mp < 10 ? mp+3 : mp-9

In `Decalendar`, January is represented by `A` and February is represented by `B` (mnemonic: `jAn`=January, `feB`=February). @sec-dotm provides more information on how positive and negative `m` values are used in the `m+dd` format.

### Gregorian calendar and `Decalendar` date conversion {#sec-gregd}

To convert a Gregorian calendar date into a `Decalendar` `doty` date, we can use the code in @exm-date2doty, which is derived from [a C++ algorithm](http://howardhinnant.github.io/date_algorithms.html#days_from_civil) in [@hinnant2014]. First, we decrement the year if the provided Gregorian calendar `month` is January (1) or February (2) with @eq-year1 and then turn the `month` and `day` into a `doty` with @eq-doty1. The inverse function in @exm-doty2date, which converts a `Decalendar` `doty` date into a Gregorian calendar date, is also derived from [@hinnant2014]. 
