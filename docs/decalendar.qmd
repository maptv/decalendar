...
```{javascript}
#| tags: [decdate]
function date2deco(year = 1970, month = 1, day = 1) {
...
To create a `deco` timestamp, we can use the `doty2deco` or the `date2deco` functions as shown in @exm-isos. Similarly, @exm-isos also shows how to create an `isoc` timestamp with the `doty2isoc` and `doty2isoc` functions. All of these timestamp creation functions assume that the provided times are in the UTC+00:00 time zone or `Zone 0` and need to be adjusted to the provided time zone by adding the appropriate time zone offset to the timestamp. @sec-unixb and @sec-unixp further discuss timestamps and provide functions for building, parsing, and converting timestamps.
...
```{javascript}
#| tags: [date2deco]
function date2deco(year = 1969, month = 1, day = 1, hour = 0, minute = 0, second = 0) {
    return `${date2year(year, month).toString().padStart(4, '0')}+${
        date2doty(month, day).toString().padStart(3, '0')}.${
