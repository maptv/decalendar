...
The most common ISO 8601 date format is `year-mm-dd`. Known as [calendar dates](https://en.wikipedia.org/wiki/ISO_8601#Calendar_dates), this format is the current widely accepted standard for displaying Gregorian calendar dates. We can combine code from @exm-coor and @exm-gregm to convert between `Decalendar` dates and ISO 8601 calendar dates as shown in @exm-isoc.
...
### ISO 8601 week dates {#sec-isow}

The ISO 8601 week date format is 
...
The `m` in the `.m` format is the 1-digit month number. To fit all of the months in a single digit, `m` is in hexadecimal form (Base16 encoded). This means that the first 10 months are represented by the numbers 0 through 9 while the last two months of the year are represented by the letters "A" and "B" instead of numbers. The negative month numbers range from -C (-13) to -1, as shown in @tbl-gregm
