...
In the Gregorian calendar, dates are like a set of coordinates, where the month and the day-of-the-month (`dotm`) are like longitude and latitude in the [geographic coordinate system](https://en.wikipedia.org/wiki/Geographic_coordinate_system) or x and y in the [Cartesian coordinate system](https://en.wikipedia.org/wiki/Cartesian_coordinate_system). `Decalendar` provides two coordinates in one number: the day-of-the-year (`doty`). The first two digits of the `doty` are the `dek` number. `Deks` are groups of 10 days that fulfill the role of both months and weeks in `Decalendar`. The last digit of the `doty` is the day-of-the-`dek` (`dotd`) number, which serves as both the `dotm` and the day-of-the-week (`dotw`) in `Decalendar`. @tbl-coor shows the `doty` equivalents of all Gregorian calendar dates. To find a specific date in @tbl-coor, first find the month among the columns (think of the month as an x-axis value) and then find the `dotm` among the rows (the day is like a y-axis value).

::: {.callout-note collapse="true"}
#### Click here to expand the Gregorian calendar date to `doty` conversion table
...
: Gregorian calendar date to positive `doty` conversion {#tbl-coor}

:::

### Gregorian calendar date conversion {#sec-conv}

In addition to using a conversion table like @tbl-coor, we can convert between Gregorian calendar dates and `doty` numbers programmatically. The code in @exm-coor is derived from the [`days_from_civil`](http://howardhinnant.github.io/date_algorithms.html#days_from_civil) and [`civil_from_days`](http://howardhinnant.github.io/date_algorithms.html#civil_from_days) algorithms described by [Howard Hinnant](https://howardhinnant.github.io/) in [_chrono-Compatible Low-Level Date Algorithms_](http://howardhinnant.github.io/date_algorithms.html) [-@hinnant2014]. The output of the `date2doty` function is a `doty` number, while its inverse function, `doty2date`, returns an array containing the month and `dotm` numbers.
...
January and February are the last months of the `Decalendar` year. During these two months, the `Decalendar` year is 1 less than the Gregorian calendar year. The functions in @exm-coor convert between `Decalendar` and Gregorian calendar month numbers, which are shown in @tbl-gregm. To obtain a `Decalendar` year (`dy`) from a Gregorian calendar year (`gy`) and month number (`gm`), we subtract 1 from `gy` if `gm` is less than 3 ($dy = gy - [gm < 3]$). To obtain a Gregorian calendar year (`gy`) from a `Decalendar` year (`dy`) and `Decalendar` `doty` (`doty`), we add 1 to `dy` if `doty` is greater than 305 ($gy = dy + [dm > 305]$). Both of these calculations are shown in @exm-gregm.
...
```julia
function date2year(year=1969, month=1)
    year - (month < 3)
end
...
```python
def date2doty(year=1969, month=1):
    return year - (month < 3)

date2year()

def doty2date(year=1969, doty=306):
...
### ISO 8601 ordinal dates {#sec-isoo}

`Decalendar` dates (`year+day`) are most similar to [ordinal dates](https://en.wikipedia.org/wiki/ISO_8601#Ordinal_dates) (`year-day`) specified by the [ISO 8601 international standard](https://en.wikipedia.org/wiki/ISO_8601) for [dates](https://en.wikipedia.org/wiki/ISO_8601#Dates) and [times](https://en.wikipedia.org/wiki/ISO_8601#Times). In fact, the `Decalendar` `doty` number can be obtained from the ISO 8601 ordinal day number using the calculations shown in @eq-isoo1, @eq-isoo2, and @exm-isoo. These calculation shift the ordinal date by 60 days to account for the 2-month difference between `Decalendar` and the Gregorian calendar. We use the `is_leap` function from @exm-gregl in @exm-isoo to correct for the fact that leap day shifts ISO 8601 ordinal dates by 1 day in leap years. `Decalendar` `doty` numbers never change, because leap day at the end of the `Decalendar` year.

$$(isoordinal + 305 - is_leap(year)) \mod 365$$ {#eq-isoo1}

$$(doty + 60 + is_leap(year + 1)) \mod 365$$ {#eq-isoo2}
...
### ISO 8601 years {#sec-isoy}

Both `Decalendar` and ISO 8601 show [years](https://en.wikipedia.org/wiki/ISO_8601#Years) as 4-digit numbers, but unlike ISO 8601, years are not mandatory in `Decalendar` dates. As mentioned in @sec-gregm, the years in `Decalendar` and ISO 8601 ordinal dates will match except for in January and February when the `Decalendar` year will be 1 less than the ISO 8601 year. Year 0 in both `Decalendar` and ISO 8601 is 1 BCE (Before Common Era) in the Gregorian calendar. The first day of Year 0 in `Decalendar`, `0000+000`, is March 1, 1 BCE in the Gregorian calendar and `0000-061` in the ISO 8601 ordinal date format.

### ISO 8601 calendar dates {#sec-isoc}

The most common ISO 8601 date format is `year-mm-dd`. Known as [calendar dates](https://en.wikipedia.org/wiki/ISO_8601#Calendar_dates), this format is the current widely accepted standard for displaying Gregorian calendar dates. In this format, the `mm` is the double-digit month number and the `dd` is the double-digit `dotm`. As mentioned in @sec-coor, `Decalendar` uses `doty` numbers as dates instead of month and day-of-the-month (`dotm`) numbers, but if required, Gregorian calendar dates can be provided in the `.m` format (`year+m+dd`), which is very similar to the ISO 8601 calendar date format. The `.m` is described in @sec-dotm with examples provided in @sec-dotex. To be clear, the `.m` format is only used to display Gregorian calendar dates and otherwise does not play any role in `Decalendar`.

We can combine code from @exm-coor and @exm-gregm to convert between `Decalendar` dates and ISO 8601 calendar dates, as shown in @exm-isoc. `Decalendar` has its own version of the ISO 8601 calendar date format called the `.m` format
...
### ISO 8601 time zone codes {#sec-isoz}

ISO 8601 timestamps will often have a `Z` at the end (`year-mm-ddThh:mm:ssZ`). This `Z` is a [military time zone code](https://en.wikipedia.org/wiki/Military_time_zone#Description) that represents the [UTC+00:00](https://en.wikipedia.org/wiki/UTC%2B00:00) time zone, which is the basis of [Coordinated Universal Time (UTC)](https://en.wikipedia.org/wiki/Coordinated_Universal_Time). `Declock` times that are synonymous with UTC should end in `Z` or `+0` (`year+day.dddddZ` or `year+day.ddddd+0`). The `+0` means that there is no offset from UTC. Noon UTC can be written `.5Z` or `.5+0` in `Declock` and 12:00Z or 12:00+00:00 in ISO 8601. The code in @exm-isoz converts a UTC offset to a one-letter military time zone code that can be used in `Declock` or ISO 8601 times.
...
The formula for conversion of standard time to `Declock` time is shown in @eq-isot. The value of `x` in @eq-isot can be modified to obtain different units, which are displayed in @tbl-isot. To convert `Declock` time into standard time, we first convert into seconds using @eq-isot2 and the appropriate `x` value from @tbl-isot. Then, we convert the seconds into hours with @eq-isot3, hours into minutes with @eq-isot4, and minutes into seconds with @eq-isot5.

$$declock = \frac{hours}{24} + \frac{minutes}{1440} + \frac{seconds}{86400} \cdot 10^x$$ {#eq-isot1}

$$seconds = declock \cdot 86400 \cdot 10^x$$ {#eq-isot2}

$$hours = \frac{seconds}{3600} $$ {#eq-isot3}

$$minutes = \frac{hours - \lfloor hours \rfloor}{60} $$ {#eq-isot4}

$$seconds = \frac{minutes - \lfloor minutes \rfloor}{60} $$ {#eq-isot5}
...
#### Standard Time Zone to Decimal Time Zone Conversion

These units can be combined together with

### ISO 8601 recurring intervals {#sec-isoi}

The `doty` numbers in the `Decalendar` dates are essentially [zero-based indexes](https://en.wikipedia.org/wiki/Zero-based_numbering) that can be positive or negative, like in computer [programming](https://en.wikipedia.org/wiki/Zero-based_numbering#Computer_programming). Another inspiration from computer programming, is the ability to use [array slicing](https://en.wikipedia.org/wiki/Array_slicing) to create time intervals. For example, [indexing and slicing of dates and times](https://pandas.pydata.org/pandas-docs/version/1.1/user_guide/timeseries.html#indexing) is fully implemented in the [Pandas Python library](https://pandas.pydata.org/).
...
To calculate `Decalendar` dates directly from UNIX time, we can use a calculation described by [Howard Hinnant](https://howardhinnant.github.io/) in his 2021 article entitled ["chrono-Compatible Low-Level Date Algorithms"](http://howardhinnant.github.io/date_algorithms.html#civil_from_days). Briefly, the seconds (or milliseconds) in UNIX time are first converted to `days` ($days = seconds \div 86400$). Then, the `days` are used to obtain the `era` (@eq-era), `day-of-era` (@eq-doe), `year-of-era` (@eq-yoe), `year` (@eq-year2), and `doty` (@eq-doty2).
...
function unix2doty(s=0, ms=0, precision=0)
    ...
    if precision == 0
        return "$(lpad(year,4,'0'))+$(lpad(Int(round(doe - (365 * yoe + yoe / 4 - yoe / 100))),3,'0'))"
    end
    timestamp = days - floor(
        year * 365 + year / 4 - year / 100 + year / 400
    )
    doy = Int(floor(timestamp))
    return "$(lpad(year,4,'0'))+$(lpad(doy,3,'0')).$(lpad(Int(round((timestamp - doy) * 10 ^ precision)),precision,'0'))"
end
...
def unix2doty(s=0, ms=0, precision=0):
    ...
    doy = int(timestamp.__floor__())
    return f"{year:0<4}+{doy:0<3}.{str(round((timestamp - doy) * 10 ** precision)).zfill(precision)}"
...
```r
unix2doty <- function(s = 0, ms = 0, precision = 0) {
  days = s / 86400 + ms / 86400000 + 719468
  ...
  years = 1:year
  if (precision == 0) {
    return(paste0(sprintf("%04d", year), "+", sprintf("%03d", round(
                    doe - (365 * yoe + yoe / 4 - yoe / 100)))))
  }
    timestamp = days - floor(
        year * 365 + year / 4 - year / 100 + year / 400
  ...
  paste0(sprintf("%04d", year),
         "+",
         sprintf("%03d", doty),
         ".",
         sprintf(paste0("%0", precision, "d"), round((timestamp - doty) * 10 ^ precision)))
}

unix2doty(as.numeric(Sys.time()), precision = 5)
```
...
Negative `doty` numbers require two separate tables for common and leap years, @tbl-negc and @tbl-negl, because all negative `doty` numbers decrease by 1 in leap years. 


@sec-dotm provides more information on how positive and negative `m` values are used in the `m+dd` format.

### Gregorian calendar and `Decalendar` date conversion {#sec-gregd}

$$doty = (153 \cdot \frac{\begin{cases}month - 3&{\text{if } month > 2;}\\m + 9&{\text{otherwise.}}\end{cases}}{5} + d - 1$$ {#eq-doty1}
