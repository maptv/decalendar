...
The negative equivalent of `1969+306.000`, the `deco` stamp in @fig-date and @tbl-2x2, is `1970-059.000`. This negative `deco` date literally subtracts 59 days from the beginning of year 1970 to arrive at `Day 306` of 1969. The year in negative `deco` dates is always 1 greater than the year in the equivalent positive `deco` date. Immediately after midnight, the negative `doty` will changed from -59 to -58, while the positive `doty` will remain the same. At noon, the positive `deco` date will be `1969+306.5` and the negative `deco` date will be `1970-058.5`.

Both `1969+306` and `1970-059` translate to January 1, 1970 in the Gregorian calendar. The first day of `Decalendar` year 1969, , is March 1 in the Gregorian calendar. which is  and therefore  the Gregorian calendar year will match the year in the negative `deco` date, but the not the year in the positive `deco` date, in January and February.
...
The first day of the `Decalendar` year, `Day 0`, is March 1 in the Gregorian calendar. Starting the year on March 1 positions Leap Day, `Day 365` in `Decalendar` and February 29 in the Gregorian calendar, at the end of the year. `Decalendar` leap years therefore occur one year earlier than Gregorian calendar leap years. To check if a year is a `Decalendar` leap year, we add 1 to the year and proceed with an [`algorithm`](http://howardhinnant.github.io/date_algorithms.html#is_leap) described by [@hinnant2014] and shown in @eq-gregl. In @exm-gregl, this algorithm is called `year2bool` to highlight that its input is a year and its output is a boolean (true or false).
...
```{julia}
#| tags: [isoo2doty]
function isoo2year(yd="1970-001")
...
## Julian dates {#sec-jd}
 ...
 Julian dates The `Decalendar` day-of-the-era (`dote`) dates are analogous to. Both track the number of [fractional days](https://en.wikipedia.org/wiki/Decimal_time#Fractional_days) since a starting point called an epoch. The Julian date epoch is `-4713+327.5`, which is noon on November 24, 4714 BC in the Gregorian calendar and January 1, 4713 BC in the Julian calendar. The `Decalendar` epoch is `0000+000.0`, which is midnight on March 1, 1 BC in the Gregorian calendar.

Like Julian dates, `Decalendar dote` dates are very useful for date and time calculations that span more than one year. If we want to find the number of fractional days in between two dates, we first convert both dates into `dotes` and subtract the earlier `dote` from the later `dote`. @eq-dote shows how a year and a `doty` can be turned into a `dote`. To convert a `dote` back into a year and `doty`

$$dote = \lfloor doty + (365 \cdot year + \frac{year}{4} - \frac{year}{100} + \frac{year}{400}) \rfloor$$ {#eq-dote}

$$dote - doty = \lfloor (365 \cdot year + \frac{year}{4} - \frac{year}{100} + \frac{year}{400}) \rfloor$$ {#eq-dote}

$$year = \lfloor \frac{doe - \frac{doe}{1460} + \frac{doe}{36524} - \frac{doe}{146096}}{365} \rfloor + era * 400$$ {#eq-year}

Like `Decalendar`, some [Julian day variants](https://en.wikipedia.org/wiki/Julian_day#Variants) start the day at midnight. Like Swatch Internet Time, Julian dates only use a single time zone ([UTC+0](https://en.wikipedia.org/wiki/List_of_UTC_offsets#UTC%C2%B100:00,_Z)). To obtain a `Declock` time from a Julian date, we subtract the Julian day number ($\lfloor JD \rfloor$) from the Julian date ($JD$), add 0.5, and then obtaining the remainder after dividing by 1 as shown in @eq-jd.

$$time = (JD - \lfloor JD \rfloor + .5) \mod 1$$ {#eq-jd}

Bo that tracks the number of [fractional days](https://en.wikipedia.org/wiki/Decimal_time#Fractional_days) since 
