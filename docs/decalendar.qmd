...
Any `doty` can be combined with a year to form a `Decalendar` `ordinal` (`deco`). Enter a year, month, `dotm`, hour, and minute below to see 1) the equivalent `deco` (`year+day.clock`), 2) season in the Northern Hemisphere, and 3) season in the Southern Hemisphere.
