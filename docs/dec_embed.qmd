...
```{ojs}
//| echo: false
viewof startStop = interval([0, 365], {step: 1, value: [91, 273], label: 'start & stop', width: 195, format: ([start, end]) => start === end ? "" : `[${formatDecimal(start)}: ${formatDecimal(end)}]`})
```
:::
...
```{ojs}
//| echo: false
viewof step = Inputs.number([1, 365], {step: 1, label: "Step", value: 1, placeholder: "Enter step size"})
```
...
Plot.plot({
  padding: 0,
  x: {axis: "top", label: "Day-of-the-dek"},
  y: {label: "Dek"},
  color: {type: "categorical", scheme: "Category10"},
  marks: [
    Plot.cell(numbers, {x: d => d[2], y: d => d.slice(0, 2), fill: d => slice(numbers, startStop[0], startStop[1], step).includes(d), inset: 0.5}),
    Plot.text(numbers, {x: d => d[2], y: d => d.slice(0, 2), fill: "white", stroke: "black", fontSize: 20})
  ]
})
```

#### `Dotd` Observable code {#sec-ojs-code-dotd .appendix}

##### Functions {.appendix}

```{ojs}
// https://observablehq.com/@mootari/range-slider
// https://github.com/hustcc/slice.js/blob/master/src/index.js
