...
```{ojs}
//| echo: false
viewof startStop = interval([0, 365], {step: 1, value: [91, 273], label: 'start:stop', width: 195, format: ([start, end]) => start === end ? "" : `${start}:${end}`})
```
:::
...
Plot.plot({
  ...
  marks: [
    Plot.cell(dates, {x: d => d[2], y: d => d.slice(0, 2), fill: d => slice(dates, startStop[0], startStop[1], step).includes(d), inset: 0.5}),
