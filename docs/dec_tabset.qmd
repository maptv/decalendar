...
// https://observablehq.com/@fheyen/barchart-clock
{
  ...
  svg.selectAll('.tickC')
    ...
    .attr('y', d=>d%2===0? firstBarY+6:firstBarY+9)
  ...
  svg.selectAll('.timeLabel')
    .data([`${barCents}%`, `${barMils}‰`, `${barBeats}♫`])
    .enter()
    .append('text')
...
The top bar in @fig-bar-time shows `cents`[^cent] (`%`). Each `cent` is 1% of the day, which is a little less a quarter hour. The middle bar shows `mils`[^mil] (`‰`). A `mil` is a tenth of a `cent`, which is close to a minute and a half. The bottom bar displays `beats`[^beat] (`♫`). A `beat` is 1% of a `mil`, which is almost as long as a second.
...
A `Declock` `beat` can be thought of as a heart❤️beat or musical🎵[beat](https://en.wikipedia.org/wiki/Beat_(music)#:~:text=the%20beat%20is%20the%20basic%20unit%20of%20time) with a constant [rate](https://en.wikipedia.org/wiki/Heart_rate#:~:text=The%20American%20Heart%20Association%20states%20the%20normal%20resting%20adult%20human%20heart%20rate%20is%2060%2D100%20bpm.) or [tempo](https://en.wikipedia.org/wiki/Tempo#Basic_tempo_markings:~:text=Adagietto%20%E2%80%93%20slower%20than,56%E2%80%93108%C2%A0bpm) of 69.4̅ beats per minute. In addition to displaying time on clocks🕰️and⌚️watches, `beats` can be used to measure durations, such as the number of `beats` since this webpage was loaded: \${styledTickTime}.

Just like `doty` times can be split up into `cents`, `mils`, and `beats`, `doty` dates have two components called the `dek`[^dek] and the day-of-the-`dek`[^dotd] (`dotd`). The `dek` (\${setStyle(doty.slice(0, 2), d3.schemePaired\[0\])}) and `dotd` (\${setStyle(doty\[2\], d3.schemePaired\[1\])}) in `Decalendar` dates (\${setStyle(dotyDate, d3.schemePaired\[1\])}) are analogous to the month and day-of-the-month (`dotm`) in [calendar dates](https://en.wikipedia.org/wiki/ISO_8601#Calendar_dates) (`mm-dd`) and the week and day-of-the-week (`dotw`) in [week dates](https://en.wikipedia.org/wiki/ISO_8601#Week_dates) (`Www-d`).
...
As an alternative to `doty` dates, `Decalendar` can express dates in fractional years, which are similar to the [fractional days](https://en.wikipedia.org/wiki/Decimal_time#Fractional_days) used in `doty` times. Fractional year dates[^year] can be combined with years into one number and are convenient for marking periods of 73 days in the `Decalendar` year. Every 73 days, the `Decalendar` fractional year date increases by .2 (⅕). The current `Decalendar` fractional year date is \${setStyle(yearDate.toString().slice(4), d3.schemePaired\[10\])}.
