...
viewof sliceInputs = Inputs.form([
    ...
    Inputs.number([-366, 366], {step: 1, label: "step", value: 1, placeholder: "Enter step size"}),
])
...
Plot.plot({
  ...
  marks: [
    Plot.cell(dates, {x: d => d[2], y: d => d.slice(0, 2),
    fill: d => slice(dates, ...sliceInputs).includes(d) && [].concat(...spread(dates, ...spreadInputs)).includes(d) ? "green" :
    slice(dates, ...sliceInputs).includes(d) && ![].concat(...spread(dates, ...spreadInputs)).includes(d) ? "blue" :
    !slice(dates, ...sliceInputs).includes(d) && [].concat(...spread(dates, ...spreadInputs)).includes(d) ? "orange" :
    "grey", inset: 0.5}),
