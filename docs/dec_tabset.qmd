...
{
const W = width > 850 ? width * 2 / 3 : width > 650 ? width : width > 550 ? width * 1.06 : width > 450 ? width * 1.1 : width > 400 ? width * 1.14 : width * 1.18;
  const H = 156;
  const barX = 1;
  const firstBarY = 10;
  const svg = d3
...
Now it's your turn! Move the sliders🎚️next to @lst-doty to adjust the `doty`, fractional year, month, and `dotm` values and see the corresponding 1) Northern Hemisphere season, 2) Southern Hemisphere season, and 3) Zodiac sign. Try entering a special date like your birthday🎂!
...
```{ojs}
//| echo: false
viewof scrubberDoty = Scrubber(numbers, {autoplay: false, alternate: true, delay: 100, loopDelay: 500})
```
...
A year and a `doty` can be combined to form a `Decalendar` `ordinal`[^deco] (`deco`). You can enter a year, month, `dotm`, hour, and minute next to @lst-date to see the equivalent 1) `deco` timestamp (`year+day.clock`) and 2) year date (`year.yyy`).
...
Once you feel comfortable with the `deco` format, you can type a `deco` next to @lst-deco to see what it looks like after it is 1) **parsed** from a string of characters into a year and `doty`, 2) **standardized** to ensure years are integers and `doties` are positive numbers below 366, and 3) **converted** into a) the year date format and b) the [ISO 8601 format](https://en.wikipedia.org/wiki/ISO_8601#Combined_date_and_time_representations) (`year-mm-ddThh:mm:ss`).
