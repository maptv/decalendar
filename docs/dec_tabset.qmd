...
// setValue(viewof yd, [yd[0], scrubberDeco])
setValue(viewof decoOutput, `${yd[0]}+${scrubberDeco}`)
```
...
In @exm-intro-deco, you can enter numeric year and `doty` values or type in a free-form `deco` to see the resulting 1) standardized `deco`, 2) year date, and 3) [ISO 8601 timestamp](https://en.wikipedia.org/wiki/ISO_8601#Combined_date_and_time_representations) (`year-mm-ddThh:mm:ss`) in @lst-intro-deco. The [number](https://observablehq.com/@observablehq/input-range#m) and [text](https://observablehq.com/@observablehq/input-text) inputs in @exm-intro-deco support negative and [fractional days](https://en.wikipedia.org/wiki/Decimal_time#Fractional_days) and years. Negative `doty` values move dates backward from the start of a given year into a previous year. Similarly, negative years move dates backward starting from `Year` `0`, which is 1 BCE (Before the [Common Era](https://en.wikipedia.org/wiki/Common_Era)). Interacting with the number inputs in @exm-intro-deco values, the `deco` displayed in @lst-intro-deco is standardized to ensure years are integers and `doty` values are positive numbers below 366.
...
1. \${styledDecoYear}+\${styledDecoDotyDate}\${decoDotyTimeDelimiter}\${styledDecoDotyTime}
...
::: {.column-margin}
```{#lst-intro-deco lst-cap="Deco string processing" cap-location=margin}
```
:::
