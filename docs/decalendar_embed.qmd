...
A timestamp that doesn't move isn't much fun, so here's one that increments every one ten thousandth ($10^{-5}$) of a day (centimilliday), which is `Decalendar` is called a `beat`. It's like a heart❤️beat!  Or if you prefer, a musical🎵beat!
...
If subtract the second timestamp from the first, we can see how long this page has been open. You don't have to read it all in one sitting, it's not a speed-reading competition, so please feel free to take breaks😄.
