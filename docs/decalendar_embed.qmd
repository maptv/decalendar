...
`Decalendar`[^decalendar] is a [decimal](https://en.wikipedia.org/wiki/Decimal) calendar🗓️system and `Declock`[^declock] is a decimal timekeeping⏳system. Instead of months, weeks, hours, minutes, and seconds, `Decalendar` and `Declock` use a number called the day-of-the-year (`doty`)[^doty]. Each `doty` (\${styledDotyDate}.\${styledDotyTime}) consists of a `Decalendar` date[^date] (\${setStyle(dotyDate, d3.schemePaired\[1\])}) and a `Declock` time[^time] (\${setStyle(dotyTime, d3.schemePaired\[2\])}). @fig-bar-time displays the current `Declock` time as a bar chart.
...
[^date]: `Decalendar` date: the integer part of the `doty`; an alternate to calendar dates and week dates
[^time]: `Declock` time: the fractional part of the `doty`; an alternate to hours, minutes and seconds

```{ojs}
//| echo: false
//| label: fig-bar-time
//| fig-cap: "Declock time bar chart"
//| fig-align: "center"
...
// https://observablehq.com/@fheyen/barchart-clock
{
  ...
  svg.selectAll('.background')
    ...
    .attr('width', W-100)
    .attr('y', (d,i)=>i*18+firstBarY)
  // Dimes/Mils
  svg
    .append('rect')
    .attr('class', 'timeBar')
    .attr('y', firstBarY)
    .attr('width', d => scaleD(Number(barMils)/100+Number(barBeats)/10000))
  svg
    .append('rect')
    .attr('class', 'timeBarFull')
    .attr('y', firstBarY)
    .attr('width', d => scaleD(barDimes))
  // Dime ticks
  svg.selectAll('.tick')
    .data(d3.range(1, 10))
    .enter()
    .append('rect')
    .attr('class', 'tick')
    .attr('x', d=>scaleD(d)+barX)
    .attr('y', d=>d%2===0?firstBarY:firstBarY)
    .attr('height', d=>d%2===0? 4:7)
  svg.selectAll('.tickLabel')
    .data(d3.range(2, 10, 2))
    .enter()
    .append('text')
    .attr('class', 'tickLabel')
    .attr('x', d=>scaleD(d)+barX+.4)
    .attr('y', firstBarY+13)
    .text(d=>d)
  // Beats
  svg
  ...
  // Beat ticks
  svg.selectAll('.tickB')
    .data(d3.range(10, 100))
    .enter()
    .append('rect')
    .attr('class', 'tickB')
    .attr('x', d=>scaleD(d/10)+barX)
    .attr('y', firstBarY+18)
    .attr('height', d=>d%2===0? 9:6)
  // Labels
  svg.selectAll('.timeLabel')
    .data([`${barMils}‰`, `${barBeats}♫`])
    .enter()
    .append('text')
    .attr('class', 'timeLabel')
    .attr('x', barX+2)
    .attr('y', (d,i)=>i*18+firstBarY+12)
    .text(d=>d)
  return svg.node();
}
```
...
The top bar in @fig-bar-time shows `mils` (`‰`), which are a thousandth (.001) of the day. The bottom bar displays `beats` (`♫`), which are like heart❤️beats or musical🎵[beats](https://en.wikipedia.org/wiki/Beat_(music)#:~:text=the%20beat%20is%20the%20basic%20unit%20of%20time) with a constant [rate](https://en.wikipedia.org/wiki/Heart_rate#:~:text=The%20American%20Heart%20Association%20states%20the%20normal%20resting%20adult%20human%20heart%20rate%20is%2060%2D100%20bpm.) or [tempo](https://en.wikipedia.org/wiki/Tempo#Basic_tempo_markings:~:text=Adagietto%20%E2%80%93%20slower%20than,56%E2%80%93108%C2%A0bpm) of 69.4̅ beats per minute. In addition to displaying time on clocks🕰️and⌚️watches, `mils` and `beats` can be used to measure durations, such as the number of `beats` since this webpage was loaded: \${styledTickTime}.

Just like `Declock` times have `mils` and `beats`, `Decalendar` `doty` dates have two components called the `dek`[^dek] and the day-of-the-`dek`[^dotd] (`dotd`). The `dek` (\${setStyle(doty.slice(0, 2), d3.schemePaired\[0\])}) and `dotd` (\${setStyle(doty\[2\], d3.schemePaired\[1\])}) in `Decalendar` dates (\${setStyle(dotyDate, d3.schemePaired\[1\])}) are analogous to the month and day-of-the-month (`dotm`) in [calendar dates](https://en.wikipedia.org/wiki/ISO_8601#Calendar_dates) (`mm-dd`) and the week and day-of-the-week (`dotw`) in [week dates](https://en.wikipedia.org/wiki/ISO_8601#Week_dates) (`Www-d`).
...
As an alternative to `doty` dates, `Decalendar` can express dates in fractional years, which are similar to the [fractional days](https://en.wikipedia.org/wiki/Decimal_time#Fractional_days) used in `Declock` times. Fractional year dates[^year] can be combined with years into one number and are convenient for marking periods of 73 days in the `Decalendar` year. Every 73 days, the `Decalendar` fractional year date increments by .2 (⅕). The current `Decalendar` fractional year date is \${setStyle(yearDate.toString().slice(4), d3.schemePaired\[10\])}.

[^year]: fractional year date: a year and the proportion of that year that has passed; an alternative to `Decalendar` dates

Now it's your turn! Move the sliders🎚️in @exm-doty to adjust the `doty`, fractional year, month, and `dotm` values and see the corresponding 1) season in the Northern Hemisphere, 2) season in the Southern Hemisphere, and 3) Zodiac sign. Try entering a special date like your birthday🎂!
...
transformInput = function(target, {bind: source, transform = identity, involutory = false, invert = involutory ? transform : inverse(transform)} = {}){
  ...
  function sourceInputHandler() {
    ...
    set(target).to(transform(source.value)).andDispatchEvent();
  ...
  function targetInputHandler() {
    ...
    set(source).to(invert(target.value)).andDispatchEvent();
...
set = (input) => ({to: (value) => (input.value = value, {andDispatchEvent: (event = new Event("input")) => input.dispatchEvent(event)})});
...
styledDek = setStyle(doty.slice(0, 2), d3.schemePaired[0])
styledDotd = setStyle(doty[2], d3.schemePaired[1])
...
styledPhrase = setStyle(dotyTime[3], d3.schemePaired[2])
...
Any `doty` can be combined with a year to form a `Decalendar` `ordinal`[^deco] (`deco`). Enter a year, month, `dotm`, hour, and minute in @exm-dt to see 1) the corresponding `deco` timestamp (`year+day.clock`), and 2) year date (`year.yyy`).
...
Once you feel comfortable with the `deco` format, you can type a `deco` in @exm-deco to see what it looks like after it is 1) **parsed** from a string of characters into a year and `doty`, 2) **standardized** to ensure years are integers and `doties` are positive numbers below 366 $\left[0, 366\right)$, and 3) **converted** into a) the `year-mm-ddThh:mm:ss` format and b) the year date format.
...
3.
  a. \${styledIsocYear}-\${styledIsocMd}T\${styledIsocTime}
  b. \${styledOutputDecoYear}
:::
...
dtDotyDateRaw = outputDatetimeDeco.slice(5, 8)
dtDotyTimeRaw = outputDatetimeDeco.slice(9, 14)
outputDatetimeYear = unix2year(utcDatetime).toString().slice(0, 8)
dtDotyTimeDelimiter = dtDotyTime ? "." : ""
ydParsed = deco2doty(inputDeco)
outputDecoYear = doty2year(...ydParsed).toString().slice(0, 8)
...
dtYear = setStyle(outputDatetimeDeco.slice(0, 4), d3.schemePaired[10])
...
As mentioned in @sec-intro-doty, `deks` function as both months and weeks in `Decalendar`. A `dek` consists of 2 groups of 5 days called `pents`[^pent]. Each `pent` can follow a sequence of workdays and restdays called a `pently` schedule[^sched]. The code in @exm-intro-dek generates @fig-schedules which compares the typical weekly schedule and the `Schedule` `3` `pently` schedule. Like other `pently` schedules, `Schedule` `3` is named after the number of workdays it contains. The 3 workdays in `Schedule` `3` are followed by a 2-day `pentend`, the `Decalender` equivalent of a weekend.
