...
`Decalendar` is a [decimal](https://en.wikipedia.org/wiki/Decimal) calendar🗓️system and `Declock` is a decimal timekeeping⏳system. Instead of months, weeks, hours, minutes, and seconds, `Decalendar` and `Declock` use a single decimal number called the day-of-the-year (`doty`). The [integer part](https://en.wikipedia.org/wiki/Decimal#:~:text=The%20integer%20part%20or%20integral%20part%20of%20a%20decimal%20numeral%20is%20the%20integer%20written%20to%20the%20left%20of%20the%20decimal%20separator) of the `doty` shows the `Decalendar` date, while the [fractional part](https://en.wikipedia.org/wiki/Decimal#:~:text=The%20part%20from%20the%20decimal%20separator%20to%20the%20right%20is%20the%20fractional%20part%2C%20which%20equals%20the%20difference%20between%20the%20numeral%20and%20its%20integer%20part.) displays the `Declock` time.

The current `doty` date and time is shown in @exm-doty. The code used to create @exm-doty is available in the [Observable](https://observablehq.com/) notebook version of this document: <https://observablehq.com/@dec/dec>.

::: {#exm-doty}
:::

```{ojs}
import { doty } from "@dec/dec"
doty
```

## Dates {#sec-summary-date}

 @fig-doty shows the `doty` number that corresponds to midnight on January 1st (`306.0`) in the [Gregorian calendar](https://en.wikipedia.org/wiki/Gregorian_calendar#:~:text=The%20Gregorian%20calendar%20is%20the%20calendar%20used%20in%20most%20parts%20of%20the%20world.). Any Gregorian calendar date and any time of day can be represented by a single `doty` number.
...
Enter a month and day in @exm-date to see the equivalent `Decalendar doty` date. Try special dates like birthdays🎂and🥂anniversaries!

::: {#exm-date}
:::

```{ojs}
import { greg2doty } from "@dec/dec"
```
...
```{ojs}
viewof dotm = Inputs.range([1, 31], {step: 1, label: "Day of the month"})
```

```{ojs}
greg2doty(month, dotm)
```

## Times {#sec-summary-time}

`Declock` measures time in [fractional days](https://en.wikipedia.org/wiki/Decimal_time#Fractional_days). The current `Declock` time is shown in @exm-time. `Declock` times are slightly more **precise** and much more **concise** than a times in hours, minutes, and seconds. The last digit of the time in @exm-time increments every one ten thousandth ($10^{-5}$) of a day (`centimilliday`), which in `Decalendar` is called a `beat`. A `beat` lasts 0.864 seconds. It's like a heart❤️beat! Or if you prefer, a musical♫beat!

::: {#exm-time}
:::

<div id="observablehq-time-4b9037e0"></div>
<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/@observablehq/inspector@5/dist/inspector.css">
<script type="module">
import {Runtime, Inspector} from "https://cdn.jsdelivr.net/npm/@observablehq/runtime@5/dist/runtime.js";
import define from "https://api.observablehq.com/@dec/dec@311.js?v=4";
new Runtime().module(define, name => { if (name === "time") return new Inspector(document.querySelector("#observablehq-time-4b9037e0")); });
</script>

## Fractional years {#sec-summary-frac-days}

`Decalendar` typically displays dates as year and `doty` integers separated by a plus sign (`year+day`), but `Decalendar` dates can also be shown as a fractional year. The current `Decalendar` date in both of these formats is shown in @exm-year

::: {#exm-year}
:::

`Doty` numbers can can be combined with a year to form a `Decalendar` ordinal (`deco`) date. @fig-year shows the `doty` number that corresponds to midnight on January 1st (`306.0`) in the [Gregorian calendar](https://en.wikipedia.org/wiki/Gregorian_calendar#:~:text=The%20Gregorian%20calendar%20is%20the%20calendar%20used%20in%20most%20parts%20of%20the%20world.).January 1st, 1970 is `1969+306`. Any `deco` date can also be represented by a fractional year.
