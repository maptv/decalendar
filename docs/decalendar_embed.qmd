...
A `doty` can represent a specific instant in a year, such as the current `doty` timestamp (`year+day.clock`): \${styledDotyDate}.\${styledDotyTime}, or a duration, such as the such as the time since this page was loaded: \${styledDotyDiff}.

`Doty` timestamps are slightly more **precise** and much more **concise** than timestamps consisting of a month, day, hour, minute, and second (`mm-ddThh:mm:ss`): \${styledIsoDate}T\${styledIsoTime}. **The `doty` is like 5 numbers in 1!** *And that's not all!*
...
You can also enter a `doty` below to get the current month and `dotm`: \${monthAndDay}. Converting back and forth between date formats is a great way to build familiarity with `doty` dates.
...
dotyDiff = (doty - dotyLoad).toString().slice(0, 7)
...
styledDotyDiff = setStyle(dotyDiff, d3.schemePaired[2])
...
styledDime = setStyle(dotyTime[0], d3.schemePaired[11])
styledCent = setStyle(dotyTime[1], d3.schemePaired[6])
styledMil = setStyle(dotyTime[2], d3.schemeAccent[5])
styledPhrase = setStyle(dotyTime[3], d3.schemePaired[8])
styledBeat = setStyle(dotyTime[4], d3.schemePaired[9])
outputDoty = setStyle(greg2doty(month, dotm).toString().padStart(3, "0"), d3.schemePaired[1])
monthAndDay = setStyle(doty2greg(inputDoty).map(i => i.toString().padStart(2, "0")).join("-"), d3.schemePaired[5]);
```

## Year {#sec-intro-year}

Any `doty` can be combined with a year to form a `Decalendar` `ordinal` (`deco`). Enter a year, month, `dotm`, hour, and minute below to see the equivalent `deco` (`year+day.ddddd`): \${dtYear}+\${dtDotyDate}\${dtDotyTimeDelimiter}\${dtDotyTime}.
...
You can also enter a `deco` to see what it looks like after it is 1) **parsed** from a string of characters into a year and `doty`, 2) **standardized** to ensure years are integers and `doties` are positive numbers between 0 and 366, and 3) **converted** into `year-mm-ddThh:mm:ss` format:
...
dtDotyDate = setStyle(outputDatetime.slice(5, 8), d3.schemePaired[1])
