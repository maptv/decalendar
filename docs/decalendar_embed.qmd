...
A `doty` can represent a specific instant in a year, such as the current `doty` timestamp (`year+day.clock`): \${styledDotyDate}.\${styledDotyTime}, or a duration, such as the time since this page was loaded: \${styledTickDate}\${tickDelimiter}\${styledTickTime}.

This timestamp and duration both increment every 864 milliseconds or $10^{-5}$ days (1 `centimilliday`), which in `Declock` is called a `beat`. There are 69.4̅ (69⁴/₉ or 625/9) `beats` in a minute, which is within the normal range for a heart❤️[rate](https://en.wikipedia.org/wiki/Heart_rate#:~:text=The%20American%20Heart%20Association%20states%20the%20normal%20resting%20adult%20human%20heart%20rate%20is%2060%2D100%20bpm.) or a musical🎵[tempo](https://en.wikipedia.org/wiki/Tempo#Basic_tempo_markings).

`Doty` timestamps and durations are more **precise** and more **concise** than timestamps and durations that include hours, minutes, and seconds. Compare the `doty` timestamp above with the current timestamp in the `mm-ddThh:mm:ss` format: \${styledIsoDate}T\${styledIsoTime}. **The `doty` is like 5 numbers in 1!** *And that's not all!*
...
The digits of the current `doty` time provide the current `dime`: \${styledDime}, `cent`: \${styledCent}, `mil`: \${styledMil}, `phrase`: \${styledPhrase}, and `beat`: \${styledBeat}. `Dimes` are used for `Declock` time zone offsets, `cents` are 1 percent of the day, `mils` provide the right level of precision for displaying time on clocks🕰️and⌚️watches, while `phrases` and `beats` are inspired by musical🎶[phrases](https://en.wikipedia.org/wiki/Phrase_(music)#:~:text=a%20phrase%20(Greek%3A%20%CF%86%CF%81%CE%AC%CF%83%CE%B7)%20is%20a%20unit%20of%20musical%20meter%20that%20has%20a%20complete%20musical%20sense%20of%20its%20own) and [beats](https://en.wikipedia.org/wiki/Beat_(music)#:~:text=the%20beat%20is%20the%20basic%20unit%20of%20time). @sec-unit provides more information on the various `Decalendar` and `Declock` units.

Now it's your turn! Enter a month and `dotm` below to see 1) the associated `doty` date, 3) season in the Northern Hemisphere, 4) season in the Southern Hemisphere, and 5) Zodiac sign. Try special dates like birthdays🎂and🥂anniversaries!
...
Similarly, you can also enter a `doty` below to get 1) the corresponding month and `dotm`, 2) season in the Northern Hemisphere, 3) season in the Southern Hemisphere, and 4) Zodiac sign. Converting back and forth between date formats is a great way to build familiarity with `doty` dates.
...
function doty2zodi(doty = 306) {
    doty = Math.floor(doty)
    return (20 <= doty && doty < 50) ? ["first", "Aries♈️"] :
     (50 <= doty && doty < 81) ? ["middle", "Taurus♉️"] :
     (81 <= doty && doty < 112) ? ["last", "Gemini♊️"] :
     (112 <= doty && doty < 144) ? ["first", "Cancer♋️"] :
     (144 <= doty && doty < 175) ? ["middle", "Leo♌️"] :
     (175 <= doty && doty < 206) ? ["last", "Virgo♍️"] :
     (206 <= doty && doty < 236) ? ["first", "Libra♎️"] :
     (236 <= doty && doty < 266) ? ["middle", "Scorpio♏️"] :
     (266 <= doty && doty < 296) ? ["last", "Sagittarius♐️"] :
     (296 <= doty && doty < 325) ? ["first", "Capricorn♑️"] :
     (325 <= doty && doty < 355) ? ["middle", "Aquarius♒️"] :
     (355 <= doty && doty <= 365) || (0 <= doty && doty < 20)
     ? ["last", "Pisces♓️"] : "Unknown"
}
```
...
outputDotyZodiac = doty2zodi(outputDoty)[1]
inputDotyZodiac = doty2zodi(inputDoty)[1]
```

```{ojs}
styledDotyDate = setStyle(dotyDate, d3.schemePaired[1])
styledDotyTime = setStyle(dotyTime, d3.schemePaired[2])
styledTickDate = (tickDate > 0) ? setStyle(tickDate, d3.schemePaired[1]) : ""
...
Any `doty` can be combined with a year to form a `Decalendar` `ordinal` (`deco`). Enter a year, month, `dotm`, hour, and minute below to see 1) the corresponding `deco` (`year+day.clock`), 2) season in the Northern Hemisphere, 3) season in the Southern Hemisphere, and 4) Zodiac sign.
...
You can also enter a `deco` to see what it looks like after it is 1) **parsed** from a string of characters into a year and `doty`, 2) **standardized** to ensure years are integers and `doties` are positive numbers below 366 $\left[0, 366\right)$, and 3) **converted** into the `year-mm-ddThh:mm:ss` format equivalent, and used to calculate the associated 4) Northern Hemisphere season, 5) Southern Hemisphere season, and 6) Zodiac sign.
...
dtDotyZodiac = doty2zodi(dtDotyDateRaw)[1]
decoDotyZodiac = doty2zodi(decoDotyDate)[1]
```
