...
Now it's your turn! Move one of the sliders🎚️in @exm-doty to change the `doty`, fractional year, month, or `dotm` value and see the corresponding 1) season in the Northern Hemisphere, 2) season in the Southern Hemisphere, and 3) Zodiac sign.
...
Any `doty` can be combined with a year to form a `Decalendar` `ordinal` (`deco`). Enter a year, month, `dotm`, hour, and minute in @exm-dt to see 1) the corresponding `deco` (`year+day.clock`), and 2) fractional year date.
