...
<link rel="stylesheet" href="ewka-regular.ttf"/>

# Summary {#sec-summary}

## Day-of-the-year {#sec-summary-doty}

`Decalendar` is a [decimal](https://en.wikipedia.org/wiki/Decimal) calendar🗓️system and `Declock` is a decimal timekeeping⏳system. Instead of months, weeks, hours, minutes, and seconds, `Decalendar` and `Declock` use a single decimal number called the day-of-the-year (`doty`). @exm-doty shows code that can be used to obtain the current `doty` date and time: ${doty}.

The [integer part](https://en.wikipedia.org/wiki/Decimal#:~:text=The%20integer%20part%20or%20integral%20part%20of%20a%20decimal%20numeral%20is%20the%20integer%20written%20to%20the%20left%20of%20the%20decimal%20separator) of the `doty` is the current `Decalendar` date: ${dotyDate}. The [fractional part](https://en.wikipedia.org/wiki/Decimal#:~:text=The%20part%20from%20the%20decimal%20separator%20to%20the%20right%20is%20the%20fractional%20part%2C%20which%20equals%20the%20difference%20between%20the%20numeral%20and%20its%20integer%20part.) of the `doty` is the current `Declock` time: ${dotyTime}.

`Doty` numbers are slightly more **precise** and much more **concise** than dates and times in the form of a month, day, hour, minute, and second. Compare the current `doty` to the current date and time in `mm-ddThh:mm:ss` format: ${isoNoYear}. **The `doty` is like 5 numbers in 1!** *And that's not all!*

The first two [digits](https://en.wikipedia.org/wiki/Numerical_digit#Computation_of_place_values) of the current `doty` date are the current `dek` number: ${dek}, while the last digit is the current day-of-the-`dek` (`dotd`): ${dotd}. `Deks` are groups of 10 days that fulfill the role of both months and weeks in `Decalendar`. The `dotd` serves as both the day-of-the-month (`dotm`) and the day-of-the-week (`dotw`) in `Decalendar`.

The digits of the current `doty` time provide the current `dime`: ${dime}, `cent`: ${cent}, `mil`: ${mil}, `phrase`: ${phrase}, and `beat`: ${beat}. `Dimes` are used for `Declock` time zone offsets, `cents` are 1 percent of the day, `mils` provide the right level of precision for displaying time on clocks🕰️and⌚️watches, `phrases` are like a musical🎶[phrase](https://en.wikipedia.org/wiki/Phrase_(music)), and `beats` are like a heart❤️beat.

::: {#exm-doty}
:::

::: {.callout-note collapse="true" title="Click to toggle code expansion" icon="false"}
```{ojs}
...
```{ojs}
dotyDate = doty.slice(0, 2)
dotyDate
```

```{ojs}
dotyTime = doty.slice(3)
dotyTime
```
...
isoNoYear = iso.slice(5, 19)
isoNoYear
```

```{ojs}
dek = dotyDate.slice(0, 2)
dek
```
...
```{ojs}
cent = dotyTime.slice(1, 3)
cent
```

```{ojs}
mil = dotyTime.slice(1, 4)
mil
```

```{ojs}
phrase = dotyTime.slice(1, 5)
phrase
```

```{ojs}
beat = dotyTime.slice(-5)
beat
```
:::

Now it's your turn! Enter a month and `dotm` below to see the equivalent `doty` date: ${outputDoty}. Try special dates like birthdays🎂and🥂anniversaries!

```{ojs}
viewof month = Inputs.range([1, 12], {step: 1, label: "Month"})
```

```{ojs}
viewof dotm = Inputs.range([1, 31], {step: 1, label: "Day"})
```

You can also enter a `doty` below to get the current month: ${monthAndDay[0]} and `dotm`: ${monthAndDay[1]}. Converting back and forth between date formats is a great way to build familiarity with `doty` dates.

```{ojs}
viewof inputDoty = Inputs.range([0, 365], {step: 1, label: "Doty"})
```
...
```{ojs}
outputDoty = greg2doty(month, dotm)
outputDoty
```

```{ojs}
monthAndDay = doty2greg(inputDoty);
monthAndDay
```
:::

## Years {#sec-summary-frac-days}

`Doty` numbers can can be combined years to form a `Decalendar` ordinal (`deco`) date (`year+day`). The current `deco` date is ${deco.slice(0,8)}. Enter a date in `year-mm-dd` format below to see the equivalent `deco` date: ${decoDate}. You can also enter a `year-mm-dd` date and `hh:mm` time to see a `deco` date and time (`year+day.ddddd`): ${decoDatetime}.


```{ojs}
viewof inputDate = Inputs.date({label: "Date", value: "1970-01-01"})
```
...
```{ojs}
decoDate = unix2deco(inputDate).slice(0, 8)
decoDate
```

```{ojs}
decoDatetime = unix2deco(inputDatetime)
decoDatetime
```

:::

Storing a date in the `Decalendar` fractional year format (`year.yyy`) allows us to avoid parsing `deco` dates. Fractional years are similar to the [fractional days](https://en.wikipedia.org/wiki/Decimal_time#Fractional_days) that make up `Declock` times. The current `Decalendar` year date is ${year}. Year dates cannot include times and are only used when it is convenient to store a `deco` date as a single number. Enter a `deco` or `year-mm-dd` date to see a custom year date. Enter a year date to turn it into a `deco` and `year-mm-dd` date.
...
year = Math.ceil(unix2year(unix) * 1e3) / 1e3
```
:::

@fig-doty shows the `doty` number that corresponds to midnight on January 1st (`306.0`) in the [Gregorian calendar](https://en.wikipedia.org/wiki/Gregorian_calendar#:~:text=The%20Gregorian%20calendar%20is%20the%20calendar%20used%20in%20most%20parts%20of%20the%20world.).

![The components of a day-of-the-year (`doty`)](fig-doty.svg){#fig-doty fig-alt="A 4-digit number with 3 digits before the decimal and 1 digit after the decimal. The three digits before the decimal are labeled 'January 1st'. The digit after the decimal is labeled 'Midnight'." fig-align="center"}

@fig-year shows the `doty` number that corresponds to midnight on January 1st (`306.0`) in the [Gregorian calendar](https://en.wikipedia.org/wiki/Gregorian_calendar#:~:text=The%20Gregorian%20calendar%20is%20the%20calendar%20used%20in%20most%20parts%20of%20the%20world.).January 1st, 1970 is `1969+306`. Any `deco` date can also be represented by a fractional year.
...
The `Decalendar` timestamp below shows the year and `doty` at the time this webpage was loaded.

::: {#observablehq-load-a38e03d2}
:::
...
A timestamp that doesn't move isn't much fun, so here's one that

::: {#observablehq-now-a38e03d2}
:::
...
If we subtract the first timestamp from the second, we can see how long this webpage has been open. You don't have to read it all in one sitting, it's not a speed-reading🏎️competition, so please feel free to take breaks😄.

::: {#observablehq-diff-a38e03d2}
:::
...
<div id="observablehq-viewof-date-1b8dfe0d"></div>
<div id="observablehq-deco-1b8dfe0d"></div>
<div id="observablehq-year-1b8dfe0d"></div>
<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/@observablehq/inspector@5/dist/inspector.css">
<script type="module">
import {Runtime, Inspector} from "https://cdn.jsdelivr.net/npm/@observablehq/runtime@5/dist/runtime.js";
import define from "https://api.observablehq.com/@dec/dec@229.js?v=3";
new Runtime().module(define, name => {
  if (name === "viewof date") return new Inspector(document.querySelector("#observablehq-viewof-date-1b8dfe0d"));
  if (name === "deco") return new Inspector(document.querySelector("#observablehq-deco-1b8dfe0d"));
  if (name === "year") return new Inspector(document.querySelector("#observablehq-year-1b8dfe0d"));
  return ["time"].includes(name);
});
</script>

### Timestamp arithmetic {#sec-demo-arith}

`Doty` numbers make working with dates and times a breeze🎐. Use the inputs below to add numbers to the current year and `doty`. Add a positive number to see a future date or add a negative number to get a past date. You can use decimal `doty` numbers for times and even decimal years for dates! ✨🤩✨ For example, the decimal year date `1969.839` ($\lceil 1969+306/365 \rceil$) is equivalent to the `doty` date `1969+306`.

## Deks {#sec-summary-dek}

![The dek components of a day-of-the-year (`doty`)](fig-deks.png){#fig-deks fig-alt="A 3-digit number with 3 additional digits after the decimal. The first two digits are labeled 'Dek', the third digit is labeled 'Day of the dek', the fourth digit is labeled 'Dimes', the fourth and fifth digits are labeled 'Cents', and all three digits after the decimal are labeled 'Mils'." fig-align="center"}
...
{{< embed schedules.ipynb#fig-schedules echo=true >}}
...
{{< embed decalendar_javascript.ipynb#greg2doty echo=true >}}

##### `doty2greg`

{{< embed decalendar_javascript.ipynb#doty2greg echo=true >}}

#### Julia

##### `greg2doty`

{{< embed decalendar_julia.ipynb#greg2doty echo=true >}}

##### `doty2greg`

{{< embed decalendar_julia.ipynb#doty2greg echo=true >}}

#### Python

##### `greg2doty`

{{< embed decalendar_python.ipynb#greg2doty echo=true >}}

##### `doty2greg`

{{< embed decalendar_python.ipynb#doty2greg echo=true >}}

#### R

##### `greg2doty`

{{< embed decalendar_r.ipynb#greg2doty echo=true >}}

##### `doty2greg`

{{< embed decalendar_r.ipynb#doty2greg echo=true >}}
...
{{< embed decalendar_javascript.ipynb#greg2year echo=true >}}

##### `doty2year`

{{< embed decalendar_javascript.ipynb#doty2year echo=true >}}

#### Julia

##### `greg2year`

{{< embed decalendar_julia.ipynb#greg2year echo=true >}}

##### `doty2year`

{{< embed decalendar_julia.ipynb#doty2year echo=true >}}

#### Python

##### `greg2year`

{{< embed decalendar_python.ipynb#greg2year echo=true >}}

##### `doty2year`

{{< embed decalendar_python.ipynb#doty2year echo=true >}}

#### R

##### `greg2year`

{{< embed decalendar_r.ipynb#greg2year echo=true >}}

##### `doty2year`

{{< embed decalendar_r.ipynb#doty2year echo=true >}}
...
{{< embed decalendar_javascript.ipynb#leap echo=true >}}

#### Julia

##### `leap`

{{< embed decalendar_julia.ipynb#leap echo=true >}}

#### Python

##### `leap`

{{< embed decalendar_python.ipynb#leap echo=true >}}

#### R

##### `leap`

{{< embed decalendar_r.ipynb#leap echo=true >}}
...
`Deco` dates (`year+day`) are very similar to [ISO 8601 ordinal (`isoo`) dates](https://en.wikipedia.org/wiki/ISO_8601#Ordinal_dates) (`year-day`). Like `Decalendar doty` numbers, `isoo` dates count the number of days since the start of the year. Unlike ordinal dates, `doty` numbers are [zero-based](https://en.wikipedia.org/wiki/Zero-based_numbering) and do not differ across common and leap years. The `deco` date can be easily obtained from the `isoo` date using the calculations shown in @eq-isoo1, @eq-isoo2, and @exm-isoo. These calculation shift the `isoo` date by 60 or 61 days to account for the 2-month difference between `Decalendar` and the Gregorian calendar.

In @exm-isoo, the `isoo2doty` and `doty2isoo` functions convert between ISO 8601 ordinal day numbers and `doty` numbers. We use the `year2bool` function from @exm-gregl in @exm-isoo to correct for the fact that Leap Day shifts ISO 8601 ordinal day numbers by 1 day in leap years. To be clear, we only have to take Leap Day into account when dealing with `isoo` dates. `Decalendar doty` numbers are the same in common and leap years, because Leap Day is at the end of the `Decalendar` year.
...
{{< embed decalendar_javascript.ipynb#isoo2doty echo=true >}}

##### `isoo2deco`

{{< embed decalendar_javascript.ipynb#isoo2deco echo=true >}}

##### `doty2isoo`

{{< embed decalendar_javascript.ipynb#doty2isoo echo=true >}}

#### Julia

##### `isoo2year`

{{< embed decalendar_julia.ipynb#isoo2doty echo=true >}}
...
{{< embed decalendar_julia.ipynb#doty2isoo echo=true >}}

#### Python

##### `isoo2year`

{{< embed decalendar_python.ipynb#isoo2year echo=true >}}

##### `isoo2doty`

{{< embed decalendar_python.ipynb#isoo2doty echo=true >}}

##### `doty2isoo`

{{< embed decalendar_python.ipynb#doty2isoo echo=true >}}

#### R

##### `isoo2year`

{{< embed decalendar_r.ipynb#isoo2year echo=true >}}

##### `isoo2doty`

{{< embed decalendar_r.ipynb#isoo2doty echo=true >}}

##### `doty2isoo`

{{< embed decalendar_r.ipynb#doty2isoo echo=true >}}
...
{{< embed decalendar_javascript.ipynb#greg2deco echo=true >}}

##### `doty2isoc`

{{< embed decalendar_javascript.ipynb#doty2isoc echo=true >}}

##### `greg2isoc`

{{< embed decalendar_javascript.ipynb#greg2isoc echo=true >}}

#### Julia

##### `deco` date

{{< embed decalendar_julia.ipynb#decdate echo=true >}}

##### ISO 8601 date

{{< embed decalendar_julia.ipynb#isodate echo=true >}}

#### Python

##### `deco` date

{{< embed decalendar_python.ipynb#decdate echo=true >}}

##### ISO 8601 date

{{< embed decalendar_python.ipynb#isodate echo=true >}}

#### R

##### `deco` date

{{< embed decalendar_r.ipynb#decdate echo=true >}}

##### ISO 8601 date

{{< embed decalendar_r.ipynb#isodate echo=true >}}

:::

### Time zones {#sec-isoz}

`Isoc` dates can be combined with the [ISO 8601 time (`isot`) format](https://en.wikipedia.org/wiki/ISO_8601#Times) (`hh:mm:ss`) to form `isoc` timestamps. Likewise, `deco` dates can be combined with `Declock` times to form `deco` timestamps. Both `deco` (`year+day.dddddZ`) and `isoc` (`year-mm-ddThh:mm:ssZ`) timestamps can end in a `Z`. This `Z` is a [military time zone code](https://en.wikipedia.org/wiki/Military_time_zone#Description) that represents the [UTC+00:00](https://en.wikipedia.org/wiki/UTC+00:00) time zone, which is the basis of [Coordinated Universal Time (UTC)](https://en.wikipedia.org/wiki/Coordinated_Universal_Time).

`Deco` timestamps that are synchronized with UTC can either end in `Z` (`year+day.dddddZ`) or `+0` (`year+day.ddddd+0`). Noon UTC can be written `.5Z` or `.5+0` in `Declock` and 12:00:00Z, 120000Z, 12:00:00+00, or 120000+00 as per ISO 8601. The code in @exm-isoz converts between UTC offsets and military time zone codes. @sec-zone provides more information on `Declock` time zones.
...
{{< embed decalendar_javascript.ipynb#hour2zone echo=true >}}

##### `zone2hour`

{{< embed decalendar_javascript.ipynb#zone2hour echo=true >}}

#### Julia

##### `hour2zone`

{{< embed decalendar_julia.ipynb#hour2zone echo=true >}}

##### `zone2hour`

{{< embed decalendar_julia.ipynb#zone2hour echo=true >}}

#### Python

##### `hour2zone`

{{< embed decalendar_python.ipynb#hour2zone echo=true >}}

##### `zone2hour`

{{< embed decalendar_python.ipynb#zone2hour echo=true >}}

#### R

##### `hour2zone`

{{< embed decalendar_r.ipynb#hour2zone echo=true >}}

##### `zone2hour`

{{< embed decalendar_r.ipynb#zone2hour echo=true >}}
...
{{< embed decalendar_javascript.ipynb#hmso2doty echo=true >}}

##### `hmso2isot`

{{< embed decalendar_javascript.ipynb#hmso2isot echo=true >}}

##### `doty2hmso`

{{< embed decalendar_javascript.ipynb#doty2hmso echo=true >}}
...
{{< embed decalendar_javascript.ipynb#doty2deco echo=true >}}
...
{{< embed decalendar_javascript.ipynb#hmso2deco echo=true >}}
...
{{< embed decalendar_javascript.ipynb#doty2dote echo=true >}}
...
{{< embed decalendar_julia.ipynb#doty2dote echo=true >}}

#### Python

##### `doty2dote`

{{< embed decalendar_python.ipynb#doty2dote echo=true >}}

#### R

##### `doty2dote`

{{< embed decalendar_r.ipynb#doty2dote echo=true >}}
...
{{< embed decalendar_javascript.ipynb#unix2doty echo=true >}}
...
{{< embed decalendar_julia.ipynb#unix2doty echo=true >}}

#### Python

##### `unix2doty`

{{< embed decalendar_python.ipynb#unix2doty echo=true >}}

#### R

##### `unix2doty`

{{< embed decalendar_r.ipynb#unix2doty echo=true >}}
...
{{< embed decalendar_javascript.ipynb#unix2deco echo=true >}}
...
{{< embed decalendar_julia.ipynb#unix2deco echo=true >}}

#### Python

##### `unix2deco`

{{< embed decalendar_python.ipynb#unix2deco echo=true >}}

#### R

##### `unix2deco`

{{< embed decalendar_r.ipynb#unix2deco echo=true >}}
...
{{< embed decalendar_javascript.ipynb#parse_dec echo=true >}}
