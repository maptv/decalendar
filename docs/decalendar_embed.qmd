...
`Doty` numbers can can be combined with years to form `Decalendar` ordinal (`deco`) dates (`year+day`). The current `deco` date is ${deco.slice(0,8)}. Enter a date in `year-mm-dd` format below to see the equivalent `deco` date: ${decoDate}. You can also enter a `year-mm-dd` date and `hh:mm` time to see the synonymous `deco` date and time (`year+day.ddddd`): ${decoDatetime}.
