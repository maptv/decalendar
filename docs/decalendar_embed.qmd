...
`Decalendar`[^decalendar] is a [decimal](https://en.wikipedia.org/wiki/Decimal) calendar🗓️system and `Declock`[^declock] is a decimal timekeeping⏳system. Instead of months, weeks, hours, minutes, and seconds, `Decalendar` and `Declock` use a number called the day-of-the-year (`doty`)[^doty].
...
The current `doty`, \${styledDotyDate}.\${styledDotyTime}, consists of the current `Decalendar` date[^date] (\${setStyle(dotyDate, d3.schemePaired\[1\])}) and the current `Declock` time[^time] (\${setStyle(dotyTime, d3.schemePaired\[2\])}).
...
// https://observablehq.com/@fheyen/barchart-clock
{
  ...
  const scaleD = d3.scaleLinear().domain([0, 10]).range(xRange);
  const scaleMandB = d3.scaleLinear().domain([0, 100]).range(xRange);
  // Background bars to show where 100% lies
  svg
    .selectAll('.background')
    .data(['dimes', 'mils', 'beats'])
    .enter()
    .append('rect')
    .attr('class', 'background timeBar')
    .attr('width', W-100)
    .attr('y', (d,i)=>i*20+firstBarY)
  // Dimes
  svg
    .append('rect')
    .attr('class', 'timeBar')
    .attr('y', firstBarY)
    .attr('width', d => scaleD(Number(barDimes)+Number(barMils)/100+Number(barBeats)/10000))
  svg
    .append('rect')
  ...
  svg.selectAll('.tick')
    ...
    .attr('height', d=>d%2===0? 4:6)
  svg.selectAll('.tickMbLabel')
    .data(d3.range(2, 10, 2))
    .enter()
    .append('text')
    .attr('class', 'tickMblabel')
  ...
  // Mils and beats ticks
  svg.selectAll('.tickMb')
    .data(d3.range(1, 10))
    .enter()
    .append('rect')
    .attr('class', 'tickMb')
  ...
  svg.selectAll('.tickMbLabel')
    ...
    .append('text')
    .attr('class', 'tickMblabel')
    .attr('x', d=>scaleMandB(d)*10+barX+.4)
    .attr('y', firstBarY+33)
    .text(d=>d*10)
  svg.selectAll('.tick2')
    ...
    .attr('y', firstBarY+40)
    .attr('height', d=>d%2===0? 8:6)
  // Labels
  svg.selectAll('.timeLabel')
    .data([`${barDimes}⅒`, `${barMils}‰`, `${barBeats}♫`])
    .enter()
    .append('text')
    .attr('class', 'timeLabel')
    .attr('x', barX+30)
    .attr('y', (d,i)=>i*20+firstBarY+12)
    .text(d=>d)
  return svg.node();
}
html`<style>
...
The first two digits of the `Decalendar` date are the `dek`[^dek] number (\${setStyle(doty.slice(0, 2), d3.schemePaired\[0\])}), while the last digit is the `dotd`[^dotd] number (\${setStyle(doty\[2\], d3.schemePaired\[1\])}).
...
The `dek` and `dotd` numbers in `Decalendar` dates are analogous to the month and day-of-the-month (`dotm`) in [calendar dates](https://en.wikipedia.org/wiki/ISO_8601#Calendar_dates) (`mm-dd`) and the week and day-of-the-week (`dotw`) in [week dates](https://en.wikipedia.org/wiki/ISO_8601#Week_dates) (`Www-d`).

`Decalendar` dates can be also expressed as fractional years, which are similar to the [fractional days](https://en.wikipedia.org/wiki/Decimal_time#Fractional_days) used in `Declock` times. The current `Decalendar` fractional year date[^year] is \${setStyle(yearDate.toString().slice(4), d3.schemePaired\[10\])}.
...
Any `doty` can be combined with a year to form a `Decalendar` `ordinal`[^deco] (`deco`). Enter a year, month, `dotm`, hour, and minute in @exm-dt to see 1) the corresponding `deco` (`year+day.clock`), and 2) fractional year date.
